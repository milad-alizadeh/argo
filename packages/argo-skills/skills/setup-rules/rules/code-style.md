...
How code reads on the page. Shape, not syntax, so it binds every language in the repo; the
`<language>-style.md` beside this file names how each language spells a rule.

**The arithmetic is a gate, not a rule here.** Function length, complexity, parameter count,
file length, nested conditionals, `else` after a return, unused code and checker-silencing
pragmas fail the build in {{LINT_CONFIG}}, every rule an error. Write to those caps; when one
fires, fix the code or ratchet the exemption where the config keeps it, never suppress
inline. What follows is only what no linter can see.

## Branch on a closed set with the exhaustive construct

When the condition is one of a known, closed set of values, use the construct the checker
verifies for exhaustiveness; the binding names it and how a missed case fails. Reserve chained
conditionals for genuinely open conditions: range checks, truthiness, arbitrary booleans. The
reason is the next variant: an exhaustive construct fails loudly when a case is added, a chain
silently falls through and ships the bug.
...
are the only exemptions from the length gate, and the binding names which kinds those are.
