...
- An entry point (a route, a handler, a CLI command, an event listener) parses its input,
  calls ONE named unit, and maps the result. **Forbidden:** business rules, branching on
  domain state, or a query inside the handler. Test: could you exercise this behaviour
  without the transport?

## Compute where the data lives

Work runs in the tier that owns the authoritative data; every other tier transports state
rather than authoring it. **Forbidden:** the same rule implemented in two tiers, and a
round-trip that exists only to format something. Move work outward only for a stated reason
(latency, privacy, offline), named where you opt out.
