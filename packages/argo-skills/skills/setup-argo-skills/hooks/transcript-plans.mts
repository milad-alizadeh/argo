...
/** The tool calls one transcript line made, and whether a sub-agent made them. */
export type ToolCallsMade = { names: string[]; sidechain: boolean }

/** One `message.content[]` block. Only a `tool_use` block carries a `name`. */
 ...
 */
export function toolCallsIn(line: string): ToolCallsMade | null {
  if (!line.startsWith('{')) return null
  let record: { message?: { content?: unknown }; isSidechain?: unknown }
  try {
...
export function writesSessionList(made: ToolCallsMade): boolean {
