 ...
 */
export const tokenize = (segment: string): string[] =>
  segment
    .trim()
    .split(/\s+/)
 ...
 * match or a miss.
 */
export const unexpanded = (token: string): boolean => token.includes('$') || token.includes('`')
 ...
 */
export function invocation(tokens: string[]): {
  prefix: string[]
 ...
 * Takes the arguments as `invocation()` returned them, and gives back the subcommand onwards.
 */
