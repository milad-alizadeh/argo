...
//
// Split from guards.test.mts on file length alone. The convention: docs/agents/worktrees.md.
import assert from 'node:assert/strict'
import { test } from 'node:test'
import { decideEdit } from './worktree-edit-guard.mts'

const ROOT = '/repo'
const WORKTREE = '/repo/.claude/worktrees/ticket-30-session-screen'

const commit = (command: string, cwd: string = ROOT) => ({
  toolName: 'Bash',
  toolInput: { command },
  command,
  cwd,
