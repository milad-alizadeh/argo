...
// The hooks travel with the skill that installs them, so their depth below the root is not fixed.
const repository = execFileSync('git', ['rev-parse', '--show-toplevel'], {
  cwd: hookDirectory,
  encoding: 'utf8',
}).trim()
const hook = path.join(hookDirectory, 'worktree-guard.mts')

/** As much of a projected harness config as this suite reads back. */
type HarnessConfiguration = {
  hooks: { PostToolUse: { matcher?: string; hooks: { command: string }[] }[] }
}

function fixture(context: TestContext, name: string) {
...
function runHook(primary: string, worktree: string, isAgent = true) {
...
test('both harnesses copy skills after a successful Bash tool', () => {
  for (const relative of ['.claude/settings.json', '.codex/hooks.json']) {
    ...
    const handlers = (configuration as HarnessConfiguration).hooks.PostToolUse.flatMap(
      (group) => group.hooks,
    )
    assert.equal((configuration as HarnessConfiguration).hooks.PostToolUse[0]?.matcher, 'Bash')
    assert.equal(
      handlers.some((handler) => handler.command.includes('worktree-guard.mts')),
      true,
    )
  }
})
