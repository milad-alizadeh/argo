// Which files a shell command line writes. Read by worktree-guard.mts, which decides what to do
...
const WRITE_COMMANDS: { all: string[]; last: string[] } = {
...
const IN_PLACE: Record<string, string[] | undefined> = {
  sed: ['-i'],
  perl: ['-i'],
  ruby: ['-i'],
  gsed: ['-i'],
...
function commandTokens(tokens: string[]): string[] {
  let i = 0
  const noise = (token: string | undefined) =>
...
function commandTargets(tokens: string[]): string[] {
  ...
  const inPlaceFlags = IN_PLACE[bare]
  if (inPlaceFlags) {
    const inPlace = rest.some((token) => inPlaceFlags.some((flag) => token.startsWith(flag)))
