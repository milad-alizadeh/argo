...
import { decide as decidePublish } from './pr-ownership-guard.mts'
...
const bash = (command: string) => ({ toolName: 'Bash', toolInput: { command }, isAgent: true })
...
test('every command ship/SKILL.md tells the agent to run survives its own guard', () => {
  // hooks.json, pr-ownership-guard.mts and AGENTS.md all assert that ship/SKILL.md writes the
