...
// How an entry's skill selection reads in the progress line.
function describeSelection(entry, exclude) {
  if (entry.skills && entry.skills !== '*') return ` [${[].concat(entry.skills).join(', ')}]`
  if (exclude.length) return ` [* minus ${exclude.join(', ')}]`
  return ''
}
...
for (const entry of entries) {
  ...
  const detail = describeSelection(entry, exclude)
