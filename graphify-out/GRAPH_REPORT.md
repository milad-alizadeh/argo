# Graph Report - argo  (2026-08-11)

## Corpus Check
- 638 files · ~266,134 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4693 nodes · 9072 edges · 343 communities (309 shown, 34 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 453 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `c42971f2`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- App Startup & Transcript Engine
- Transcript Content Blocks
- Transcript Record Parsing
- Monorepo Root Tooling Config
- Transcript Event Description & Errors
- Hub Connection & Usage State
- Tool Call Events
- Color System & Palette
- Diff Evidence Model
- Cockpit Rooms & Deck Zones
- Turbo Build Pipeline Config
- File Tailing & Watching
- Motion & Animation Tokens
- Session Roster Projection
- Agent Plan & Tool Reading
- Git Checkout Projection
- Cockpit Shell View Scaffolding
- Turbo Task Config
- Typography Tokens
- Cockpit View & Actions
- Sessions Deck UI
- Specimen Deck Fixtures
- Specimen Catalog
- macOS Package Scripts
- Elevation & Shadow Tokens
- Connection Status Chip
- Contract Specimen
- Geometry & Spacing Tokens
- Git Status Vessel
- Swift Package Manifest
- Project Strip UI
- Session Navigator UI
- Shell Sidebar UI
- Layout Constants
- Testing
- AgentTerminals
- EvidenceLanguage
- MarkdownBlock
- SessionDiscovery
- Session
- MediaProvenance
- .call
- CompanionConnection
- ClaimID
- observe
- FeedCall
- Content
- ProseLinkRun
- HubJoin
- WorkspaceProjection
- FeedRowSelection
- Equatable
- String
- .rows
- CockpitCoordinator.swift
- FeedE2ECase
- FeedProseText
- .spend
- .invite
- SessionStatus
- SwiftTermAgentProcess
- SessionComposerProjection
- Context
- Sendable
- EvidenceReading
- RosterE2ECase
- ChangeSink
- ResolvedCall
- CompanionClient
- HubTestObservation.swift
- FeedCallReading
- ArgoTypeScale
- FeedFixture
- .run
- CockpitCoordinator
- ProjectRegistryStore
- ToolCallOutcome
- AgentLauncher
- HandoffChain
- HubSession
- FakeHandoffHost
- SpawnFixture
- CockpitActions
- SessionAnnotations
- CheckoutFixture
- CockpitPresentation.Session
- .header
- ArgoIconSize
- XCTestCase
- CompanionFact
- Engine
- ProjectRecord
- TranscriptObservation
- DeckContentRow
- Identifiable
- FeedShot
- Position
- XCTest
- CompanionOutcome
- TranscriptRecord
- Kind
- FeedView
- Ask
- ToolCall
- ProjectFixture
- CockpitRoom
- FeedCallRun.swift
- SessionsDeck
- SessionCommands
- ArgoColor
- .session
- LaunchConfiguration
- SessionAnnotationStore
- ProcessLivenessReader
- Hub
- ToolCallKind
- AgentSpawnError
- AgentProcessEvents
- ArgoGlyph
- FeedMarkdownTable
- PlanPillE2ETests
- Codable
- .spawnSession
- AgentSpawn
- Project
- .figure
- RosterOrder
- SessionRow
- ProjectDrawerProjection
- ArgoTheme
- HandoffWait
- .briefURL
- ProjectRegistry+Codable.swift
- SessionLiveness
- StopReason
- .promptEvents
- diffEvidence
- firstLines
- CockpitView
- DeckSeam
- EvidenceHunk
- EvidenceHeader
- FeedCallSubject
- FeedQuietCommand
- FeedSurvey
- FeedProjection
- FeedRowButtonStyle
- FeedTail
- .step
- PlanPill
- .rename
- CockpitCoordinator
- CodingKeys
- CompanionChannel
- .materialize
- Data
- HubObservation
- CheckoutReader
- SessionProvenance
- .lines
- AnnotationFile
- rosterOf
- .showed
- DeckZone
- FeedAgents
- FeedProse
- InstrumentDeckShell
- .labels
- NewSessionE2ETests
- ProjectDrawerE2ETests
- RosterOrderE2ETests
- describe
- HubConnection
- TestGate
- CockpitPresentation.Session
- .folded
- .pin
- FeedUnreadable
- SessionHeader
- NewSessionOffer
- SessionSpendFixture
- FeedHandoffTests.swift
- ArgoApp
- LaunchProjectTests.swift
- FeedSwitchE2ETests
- Hub
- Entry
- LaunchProject
- .read
- send
- ComposerMode
- .call
- FeedAskOption
- FeedCallLine
- FeedView
- FeedShotView
- ContextBar
- SessionHeaderProjection.Context.Tier
- RosterSearch
- .swatches
- FeedNewMessagesTests.swift
- ProjectProjectionTests.swift
- SessionHeaderContextTests.swift
- header
- header
- ContextGuideE2ETests
- FeedOpeningE2ETests
- .beginDiscovery
- Tier
- AgentLaunch
- SessionAnnotationStoreTests.swift
- CockpitPresentation.Session
- ComposerFooter
- SendButton
- FeedAskQuestion
- .latch
- .move
- PlanStepLine
- .phrase
- NewSessionButton
- ChurningRosterSpecimen
- FoundationSpecimen
- SessionHeaderSpecimen
- ArgoFeedRow.swift
- EvidenceAddressTests.swift
- MarkedProseTests.swift
- RosterSearchTests.swift
- SessionRenameProjectionTests.swift
- Group
- CockpitCoordinator+Send.swift
- FeedWalkE2ETests
- Usage
- AgentLauncherTests.swift
- CheckoutReaderTests.swift
- HandoffChainTests.swift
- answer
- HubHandoffTests.swift
- ProcessLivenessReaderTests.swift
- SessionOwnershipTests.swift
- ComposerField
- ComposerSeam
- Hashable
- FeedTailButton
- SessionHeaderContext
- ProjectVessel
- SpawningRosterSpecimen
- ArgoBadge
- ArgoOpacity
- EvidenceSyntaxTests.swift
- FeedScaleTests.swift
- session
- SessionHeaderProjectionTests.swift
- argo
- .liveCwds
- SessionSpendTests.swift
- AgentCLI+ReadableName.swift
- CockpitPresentation+PreviewFanout.swift
- CockpitPresentation+PreviewTranscript.swift
- AgentChip
- FeedGalleryRow
- Usage
- SpecimenScreen
- ArgoMarkedName
- presentation
- SessionRosterArchiveTests.swift
- SessionRosterProjectionTests.swift
- NarrationReadingTests.swift
- ArgoSymbol.swift
- ClaudeSessionDriver
- ProjectRecord
- ComposerDraft
- .attachments
- PermissionPromptFooter.swift
- WrapFlow
- ArgoFloatingGlass
- ConnectionFault
- RecordDirectoryFixture
- ComposerSeamNote
- AccountBindingReference
- AttachmentStore
- PermissionExpiry
- ConnectChallenge
- ConnectPortState
- .decode
- ComposerDrafts
- SessionsDeck
- AccountsCoordinator
- AttachedTerminal
- ConnectPortRow
- QueuedTurn
- EvidenceListing
- FeedTableView
- .sessions
- .connection
- GitHubAuthorization
- resolvedPath
- FeedTableDelta
- .contains
- PermissionPromptTarget
- .allows
- FeedTableHandle
- SessionState
- ToolbarSegment
- ProjectSettingsCommands
- AttachButton.swift
- AttachmentProjection
- EvidenceSource
- FeedRowCell
- .makeScrollView
- .rename
- AccountProvider
- ProjectBinding
- .keystrokes
- .phrase
- .heading
- AttachmentChip
- StandingAllowTray
- .isWorking
- FeedLightbox
- FeedMarkdownFence
- SessionHandoffButton
- GuideLine
- EvidenceStepTests.swift
- SessionAttachmentTests.swift
- LivePermissionTests.swift
- ConnectNote
- ConnectRow
- Benefit
- AttachmentTray
- WallClock.swift
- ConnectCopyTests.swift
- Row
- HubSessionEventsTests.swift
- LaunchProjectTests.swift
- SessionContextTests.swift
- AccountPort+ReadableName.swift
- AccountProvider+ReadableName.swift
- ConnectionCause+ReadableName.swift
- ComposerPasteboardTests.swift
- EvidenceLanguageTests.swift
- session
- permission-hook.sh

## God Nodes (most connected - your core abstractions)
1. `ArgoEngine` - 248 edges
2. `Foundation` - 240 edges
3. `SwiftUI` - 167 edges
4. `Testing` - 146 edges
5. `Specimen` - 111 edges
6. `ArgoUI` - 84 edges
7. `JSONValue` - 65 edges
8. `Hub` - 43 edges
9. `AccountRecord` - 35 edges
10. `ToolCallOutcome` - 34 edges

## Surprising Connections (you probably didn't know these)
- `AccountsCoordinator` --calls--> `ConnectionHealthLedger`  [INFERRED]
  apps/macOS/Argo/AccountsCoordinator.swift → apps/macOS/Packages/ArgoEngine/Sources/ArgoEngine/Health/ConnectionHealthLedger.swift
- `ArgoApp` --calls--> `CockpitNavigationModel`  [INFERRED]
  apps/macOS/Argo/ArgoApp.swift → apps/macOS/Packages/ArgoUI/Sources/ArgoUI/Shell/CockpitNavigationModel.swift
- `probe()` --calls--> `AccountGrant`  [INFERRED]
  apps/macOS/Packages/ArgoEngine/Tests/ArgoEngineTests/BindingScopeCheckTests.swift → apps/macOS/Packages/ArgoEngine/Sources/ArgoEngine/Account/AccountGrant.swift
- `ConnectFixture` --calls--> `AccountRecord`  [INFERRED]
  apps/macOS/Packages/ArgoUI/Sources/ArgoUI/Specimen/ConnectFixture.swift → apps/macOS/Packages/ArgoEngine/Sources/ArgoEngine/Account/AccountRecord.swift
- `ConnectionHealthSpecimen` --calls--> `AccountRecord`  [INFERRED]
  apps/macOS/Packages/ArgoUI/Sources/ArgoUI/Specimen/ConnectionHealthSpecimen.swift → apps/macOS/Packages/ArgoEngine/Sources/ArgoEngine/Account/AccountRecord.swift

## Import Cycles
- None detected.

## Communities (343 total, 34 thin omitted)

### Community 0 - "App Startup & Transcript Engine"
Cohesion: 0.11
Nodes (16): Hub, Hub, Bool, CompanionReport, HubSession, Int, Never, SessionOwnership (+8 more)

### Community 1 - "Transcript Content Blocks"
Cohesion: 0.11
Nodes (18): ask(), options(), question(), String, JSONValue, array, bool, null (+10 more)

### Community 2 - "Transcript Record Parsing"
Cohesion: 0.11
Nodes (18): String, TranscriptEvent, TranscriptContextCursor, AsyncStream, ImageReader, TranscriptEvent, URL, transcriptEvents() (+10 more)

### Community 3 - "Monorepo Root Tooling Config"
Cohesion: 0.06
Nodes (35): @biomejs/biome, husky, jscpd, lint-staged, bin, argo, devDependencies, @biomejs/biome (+27 more)

### Community 4 - "Transcript Event Description & Errors"
Cohesion: 0.06
Nodes (26): calls(), String, calls(), ified, String, diff(), ImageReader, String (+18 more)

### Community 5 - "Hub Connection & Usage State"
Cohesion: 0.16
Nodes (11): SessionStateIndicator, SpecimenStateDot, SpecimenStatusChip, String, ArgoOperationalState, attention, failure, idle (+3 more)

### Community 6 - "Tool Call Events"
Cohesion: 0.09
Nodes (21): Int, String, Usage, TranscriptEvent, branch, compaction, cwd, headLeaf (+13 more)

### Community 7 - "Color System & Palette"
Cohesion: 0.17
Nodes (14): ArgoPalette, DiffRoles, EdgeRoles, InteractionRoles, StateRoles, SurfaceRoles, ArgoColor, String (+6 more)

### Community 8 - "Diff Evidence Model"
Cohesion: 0.11
Nodes (22): DiffEvidence, DiffHunk, DiffLine, DiffLineSide, add, context, del, FileChange (+14 more)

### Community 9 - "Cockpit Rooms & Deck Zones"
Cohesion: 0.19
Nodes (12): DeckTab, activity, delivery, FeedEntry, Room, code, sessions, work (+4 more)

### Community 10 - "Turbo Build Pipeline Config"
Cohesion: 0.08
Nodes (24): ARGO_LIVE_CLI, ^build, dist/**, OPENAI_API_KEY, out/**, REALTIME_MODEL, dependsOn, outputs (+16 more)

### Community 11 - "File Tailing & Watching"
Cohesion: 0.12
Nodes (15): FileCursor, FileWatcher, AsyncStream, FileHandle, String, URL, Void, transcriptLines() (+7 more)

### Community 12 - "Motion & Animation Tokens"
Cohesion: 0.15
Nodes (15): Animation, ArgoAnimationModifier, ArgoMotion, Curve, easeInOut, easeOut, spring, Bool (+7 more)

### Community 13 - "Session Roster Projection"
Cohesion: 0.31
Nodes (7): SessionRenameProjection, SessionRosterProjection, Row, SessionRosterProjection, Bool, CockpitPresentation, String

### Community 14 - "Agent Plan & Tool Reading"
Cohesion: 0.11
Nodes (21): Plan, PlanEntry, PlanEntryStatus, completed, inProgress, pending, ToolUseBlock, Entry (+13 more)

### Community 15 - "Git Checkout Projection"
Cohesion: 0.08
Nodes (20): AccountGrant, AccountGrantLifetime, expiring, nonExpiring, String, AccountGrantStore, AccountRegistryStore, AccountRegistry (+12 more)

### Community 16 - "Cockpit Shell View Scaffolding"
Cohesion: 0.04
Nodes (30): AppKit, View, PermissionPromptHeader, EvidenceCopyButton, EvidenceStepHeader, FeedEvidence, String, HighlightColors (+22 more)

### Community 17 - "Turbo Task Config"
Cohesion: 0.13
Nodes (14): cache, outputs, extends, $schema, cache, outputs, persistent, tasks (+6 more)

### Community 18 - "Typography Tokens"
Cohesion: 0.17
Nodes (11): ArgoMarkedName, String, ArgoTextStyle, ArgoTypeface, interface, machine, CGFloat, Font (+3 more)

### Community 19 - "Cockpit View & Actions"
Cohesion: 0.14
Nodes (9): CockpitNavigationModel, CockpitPresentation, CockpitSpawn, CockpitPresentation, NewSessionCommands, CockpitPresentation, SpawningRosterSpecimen, String (+1 more)

### Community 20 - "Sessions Deck UI"
Cohesion: 0.05
Nodes (38): ConnectNoteView, ConnectNote, Void, WelcomeScreen, DeckSeparator, FeedAskLine, FeedAskQuestion, ArgoColor (+30 more)

### Community 21 - "Specimen Deck Fixtures"
Cohesion: 0.43
Nodes (5): SpecimenDeck, SpecimenDeckTabs, SpecimenFeedEntry, CockpitPresentation, SpecimenFixtures

### Community 22 - "Specimen Catalog"
Cohesion: 0.02
Nodes (101): Specimen, archivedRoster, churningRoster, composer, composerAttached, composerCeiling, composerDraftKept, composerDragOver (+93 more)

### Community 23 - "macOS Package Scripts"
Cohesion: 0.20
Nodes (9): description, name, private, scripts, build, lint, screenshot, test (+1 more)

### Community 24 - "Elevation & Shadow Tokens"
Cohesion: 0.11
Nodes (15): ProjectBindingIndex, Refused, AccountRegistryStore, BindingFixture, ProjectBinding, ProjectMissing, Question, RecordingScopeCheck (+7 more)

### Community 25 - "Connection Status Chip"
Cohesion: 0.09
Nodes (17): GitHubDeviceFlow, GitHubDeviceChallenge, String, GitHubDeviceFlow, Data, GitHubDeviceChallenge, GitHubOAuthApp, HTTPTransport (+9 more)

### Community 26 - "Contract Specimen"
Cohesion: 0.20
Nodes (6): ContractSpecimen, ArgoColor, String, View, ContractSpecimen, String

### Community 27 - "Geometry & Spacing Tokens"
Cohesion: 0.53
Nodes (5): ArgoRadius, ArgoSpacing, ArgoStroke, CGFloat, String

### Community 28 - "Git Status Vessel"
Cohesion: 0.40
Nodes (4): GitVessel, CockpitPresentation, String, Void

### Community 30 - "Project Strip UI"
Cohesion: 0.05
Nodes (8): SessionStatusTests, StandingAllowChannelTests, CockpitPresentation.Session, Self, RenameFixture, ArgoEngine, Foundation, SwiftTerm

### Community 31 - "Session Navigator UI"
Cohesion: 0.13
Nodes (13): SessionNavigator, Binding, Bool, CockpitPresentation, SessionRosterProjection, String, Void, SessionRow (+5 more)

### Community 32 - "Shell Sidebar UI"
Cohesion: 0.09
Nodes (22): BindingFault, accountRemoved, grantExpired, grantMissing, portNotServedByProvider, BindingRefusal, grantExpired, noGrant (+14 more)

### Community 33 - "Layout Constants"
Cohesion: 0.31
Nodes (5): CGFloat, ClosedRange, ArgoLayout, CGFloat, ClosedRange

### Community 34 - "Testing"
Cohesion: 0.05
Nodes (13): fileName(), FeedCall, FeedCommandLineTests, FeedNarrationTests, FeedPermissionExpiryTests, FeedTailContractTests, MarkdownBlockTests, PlanPillContractTests (+5 more)

### Community 35 - "AgentTerminals"
Cohesion: 0.20
Nodes (9): Adopted, AgentTerminals, Bool, Int, SessionOwnership, String, UInt8, Void (+1 more)

### Community 36 - "EvidenceLanguage"
Cohesion: 0.12
Nodes (15): EvidenceLanguage, css, golang, html, javascript, json, markdown, python (+7 more)

### Community 37 - "MarkdownBlock"
Cohesion: 0.09
Nodes (22): MarkdownBlock, bullet, fenced, heading, numbered, paragraph, table, MarkdownScan (+14 more)

### Community 38 - "SessionDiscovery"
Cohesion: 0.17
Nodes (10): SessionDiscovery, AgentCLI, Bool, String, TimeInterval, URL, AgentCLI, Bool (+2 more)

### Community 39 - "Session"
Cohesion: 0.19
Nodes (16): Access, external, managed, orphaned, CockpitPresentation, Issue, Session, AgentCLI (+8 more)

### Community 40 - "MediaProvenance"
Cohesion: 0.06
Nodes (34): CockpitPresentation.Session, PreviewShot, String, Tier, TranscriptEvent, MediaEvidence, MediaPicture, CGSize (+26 more)

### Community 41 - ".call"
Cohesion: 0.06
Nodes (40): Usage, handedOver(), sameBrief(), TranscriptEvent, asked(), String, edits(), Int (+32 more)

### Community 42 - "CompanionConnection"
Cohesion: 0.19
Nodes (9): CompanionConnection, Bool, DispatchSourceRead, Int32, String, UInt8, Void, DispatchSourceWrite (+1 more)

### Community 43 - "ClaimID"
Cohesion: 0.24
Nodes (8): SessionOwnership, Int, String, Claim, ClaimID, SessionOwnership, Int, String

### Community 44 - "observe"
Cohesion: 0.08
Nodes (21): Data, FileHandle, String, URL, TranscriptOrigin, DeckMiddleCut, Int, String (+13 more)

### Community 45 - "FeedCall"
Cohesion: 0.12
Nodes (23): Churn, Disclosure, available, none, Ending, failed, pending, succeeded (+15 more)

### Community 46 - "Content"
Cohesion: 0.13
Nodes (15): Content, ask, call, gallery, mark, message, prompt, survey (+7 more)

### Community 47 - "ProseLinkRun"
Cohesion: 0.12
Nodes (19): ProseCode, ProseLink, ProseLinkRun, ProseLinkTarget, ProseRenderer, ArgoColor, CGRect, Text (+11 more)

### Community 48 - "HubJoin"
Cohesion: 0.18
Nodes (11): HubJoin, Bool, HubSession, Int, String, TranscriptEvent, HubSessionChain, HubTranscript (+3 more)

### Community 49 - "WorkspaceProjection"
Cohesion: 0.14
Nodes (16): Kind, main, worktree, Int, String, WorkspaceProjection, Bool, GitCommand (+8 more)

### Community 50 - "FeedRowSelection"
Cohesion: 0.13
Nodes (13): FeedRow, FeedRowSelection, Bool, FeedRow, Int, Void, FeedRow, FeedRowView (+5 more)

### Community 51 - "Equatable"
Cohesion: 0.13
Nodes (21): AccessMark, Checkout, SessionHeaderProjection, AgentCLI, CockpitPresentation, Int, String, Handoff (+13 more)

### Community 52 - "String"
Cohesion: 0.12
Nodes (18): CompanionTool, askUser, reportOutcome, reportStatus, Any, MediaEvidence, OutputEvidence, Tier (+10 more)

### Community 53 - ".rows"
Cohesion: 0.18
Nodes (9): ClaudeInterrupt, Bool, String, FeedProjection, Bool, FeedRow, String, TranscriptEvent (+1 more)

### Community 54 - "CockpitCoordinator.swift"
Cohesion: 0.13
Nodes (9): LiveCLI, settle(), shell(), Bool, String, UInt8, text(), ArgoTerminal (+1 more)

### Community 55 - "FeedE2ECase"
Cohesion: 0.16
Nodes (9): FeedE2ECase, CGFloat, Int, String, XCUIElement, FeedEndE2ETests, String, FeedMotionE2ETests (+1 more)

### Community 56 - "FeedProseText"
Cohesion: 0.12
Nodes (14): FeedMarkdown, FeedMarkdownBlock, String, FeedPrompt, Bool, CGFloat, Int, String (+6 more)

### Community 57 - ".spend"
Cohesion: 0.14
Nodes (12): ElapsedTime, Int, String, SessionHeaderProjection, SpendPolicy, CockpitPresentation, Int, String (+4 more)

### Community 58 - ".invite"
Cohesion: 0.21
Nodes (10): CompanionEndpoint, Any, String, Void, CompanionRequest, CompanionResponse, Any, Bool (+2 more)

### Community 59 - "SessionStatus"
Cohesion: 0.15
Nodes (13): HubSession, SessionSignals, SessionStatus, asking, ended, idle, permission, running (+5 more)

### Community 60 - "SwiftTermAgentProcess"
Cohesion: 0.16
Nodes (9): Int, Int32, String, UInt8, SwiftTermAgentProcess, ArraySlice, LocalProcess, LocalProcessDelegate (+1 more)

### Community 61 - "SessionComposerProjection"
Cohesion: 0.23
Nodes (9): Composer, SessionComposerProjection, AgentCLI, Bool, CockpitPresentation, String, ComposerSpecimen, Bool (+1 more)

### Community 62 - "Context"
Cohesion: 0.12
Nodes (18): FeedTable, Bool, FeedRow, FeedTableCoordinator, NSScrollView, Set, Void, Context (+10 more)

### Community 63 - "Sendable"
Cohesion: 0.24
Nodes (12): ContentBlock, image, thinking, toolResult, toolUse, unreadable, ImageBlock, imageMediaType() (+4 more)

### Community 64 - "EvidenceReading"
Cohesion: 0.07
Nodes (25): EvidenceDiff, EvidenceHeader, EvidenceHeaderSpecimen, FeedEvidence, String, Void, EvidenceMedia, MediaEvidence (+17 more)

### Community 65 - "RosterE2ECase"
Cohesion: 0.18
Nodes (9): RosterE2ECase, Bool, String, TimeInterval, XCUIElement, RosterSelectionE2ETests, String, XCUIElement (+1 more)

### Community 66 - "ChangeSink"
Cohesion: 0.07
Nodes (22): ChangeSink, RecordDirectoryWatcher, AsyncStream, Bool, TimeInterval, URL, Void, AsyncStream (+14 more)

### Community 67 - "ResolvedCall"
Cohesion: 0.19
Nodes (15): diskFallbackPath(), fromContent(), fromDisk(), fromToolUseResult(), mediaEvidence(), ImageReader, MediaEvidence, String (+7 more)

### Community 68 - "CompanionClient"
Cohesion: 0.12
Nodes (19): reply(), report(), socketPath(), Any, SessionOwnership, String, Void, withChannel() (+11 more)

### Community 69 - "HubTestObservation.swift"
Cohesion: 0.10
Nodes (33): AsyncStream, String, TranscriptEvent, URL, TranscriptObservation, hub(), String, working() (+25 more)

### Community 70 - "FeedCallReading"
Cohesion: 0.19
Nodes (8): ToolCall, FeedCallReading, FeedCall, String, ToolResult, FeedPath, Bool, String

### Community 71 - "ArgoTypeScale"
Cohesion: 0.11
Nodes (17): Int, ArgoTypeScale, body, callout, caption1, caption2, footnote, headline (+9 more)

### Community 72 - "FeedFixture"
Cohesion: 0.19
Nodes (12): Pending, PermissionChannel, Bool, Int, Never, SessionOwnership, String, Task (+4 more)

### Community 73 - ".run"
Cohesion: 0.21
Nodes (12): HandoffHost, Failure, briefNeverArrived, noFolder, notSteerable, Outcome, Request, SessionHandoff (+4 more)

### Community 74 - "CockpitCoordinator"
Cohesion: 0.06
Nodes (33): CockpitCoordinator, Bool, Hub, ProjectRecord, SessionAnnotationStore, String, URL, LaunchConfiguration (+25 more)

### Community 75 - "ProjectRegistryStore"
Cohesion: 0.13
Nodes (17): HTTPBody, form, json, HTTPRequest, HTTPTransportError, malformedURL, status, unauthorized (+9 more)

### Community 76 - "ToolCallOutcome"
Cohesion: 0.11
Nodes (18): Int, ToolResult, Usage, ToolCallOutcome, ToolCallStatus, completed, failed, inProgress (+10 more)

### Community 77 - "AgentLauncher"
Cohesion: 0.18
Nodes (8): CompanionInvitation, String, AgentExecutable, String, AgentLauncher, AgentCLI, ShellCommand, String

### Community 78 - "HandoffChain"
Cohesion: 0.21
Nodes (8): HandoffChain, Link, Bool, Int, String, HandoffChainStore, Bool, URL

### Community 79 - "HubSession"
Cohesion: 0.20
Nodes (10): HubSession, AgentCLI, CompanionReport, Int, Set, String, TranscriptEvent, URL (+2 more)

### Community 80 - "FakeHandoffHost"
Cohesion: 0.18
Nodes (8): SessionSeed, String, FakeHandoffHost, HandoffFixture, Bool, String, URL, Void

### Community 81 - "SpawnFixture"
Cohesion: 0.18
Nodes (9): PermissionPatience, Int, SpawnServices, URL, SpawnFixture, Hub, LivenessRead, String (+1 more)

### Community 82 - "CockpitActions"
Cohesion: 0.09
Nodes (18): CockpitActions, Bool, Int, SessionAttachment, String, Void, ConnectionChips, CockpitPresentation (+10 more)

### Community 83 - "SessionAnnotations"
Cohesion: 0.37
Nodes (4): Annotation, SessionAnnotations, Bool, String

### Community 84 - "CheckoutFixture"
Cohesion: 0.13
Nodes (17): CheckoutProjection, Head, branch, detached, unavailable, String, URL, CheckoutFixture (+9 more)

### Community 85 - "CockpitPresentation.Session"
Cohesion: 0.28
Nodes (6): CockpitPresentation.Session, LongProse, CockpitPresentation.Session, Int, String, TranscriptEvent

### Community 86 - ".header"
Cohesion: 0.44
Nodes (5): SessionHeaderFixture, CockpitPresentation, Int, SessionHeaderProjection, String

### Community 87 - "ArgoIconSize"
Cohesion: 0.16
Nodes (11): ArgoIconSize, control, inline, Font, String, View, ArgoLabelStyle, LabelStyle (+3 more)

### Community 88 - "XCTestCase"
Cohesion: 0.16
Nodes (4): FeedGalleryE2ETests, FeedFollowingE2ETests, FeedKeyboardE2ETests, XCTestCase

### Community 89 - "CompanionFact"
Cohesion: 0.21
Nodes (7): CompanionArguments, String, CompanionFact, ask, outcome, status, CompanionReport

### Community 90 - "Engine"
Cohesion: 0.16
Nodes (10): Engine, CheckoutRead, LivenessRead, Set, String, URL, URL, TranscriptObservationError (+2 more)

### Community 91 - "ProjectRecord"
Cohesion: 0.30
Nodes (5): ProjectRegistry, AccountPort, ProjectBinding, ProjectRecord, String

### Community 92 - "TranscriptObservation"
Cohesion: 0.20
Nodes (7): Bool, URL, TranscriptFile, TimeInterval, Date, Int, timestampMs()

### Community 93 - "DeckContentRow"
Cohesion: 0.18
Nodes (14): DeckContentRow, FeedColumn, Binding, Bool, ComposerDraft, ComposerSend, FeedEvidence, FeedRow (+6 more)

### Community 94 - "Identifiable"
Cohesion: 0.17
Nodes (9): AgentChip, String, AgentsRail, Int, FeedAgent, Bool, Int, String (+1 more)

### Community 95 - "FeedShot"
Cohesion: 0.16
Nodes (12): FeedGallery, String, FeedGalleryFold, FeedRow, FeedGalleryRow, Void, FeedCall, FeedShot (+4 more)

### Community 96 - "Position"
Cohesion: 0.18
Nodes (11): Position, access, agent, checkout, issue, SessionHeaderFacts, SessionHeaderFactsGallery, Bool (+3 more)

### Community 97 - "XCTest"
Cohesion: 0.15
Nodes (6): FeedArrivalE2ETests, String, FeedSilenceE2ETests, String, SessionRenameStrings, XCTest

### Community 98 - "CompanionOutcome"
Cohesion: 0.26
Nodes (9): CompanionAsk, CompanionOutcome, CompanionReport, Bool, String, Target, artifact, code (+1 more)

### Community 99 - "TranscriptRecord"
Cohesion: 0.18
Nodes (10): String, TranscriptRecord, aiTitle, assistant, attachment, lastPrompt, queueOperation, unknown (+2 more)

### Community 100 - "Kind"
Cohesion: 0.15
Nodes (13): Kind, create, delegate, delete, edit, execute, fetch, mcp (+5 more)

### Community 101 - "FeedView"
Cohesion: 0.25
Nodes (6): FeedView, Bool, FeedRow, Set, ConnectionHealthSpecimen, State

### Community 102 - "Ask"
Cohesion: 0.24
Nodes (6): Ask, Question, String, FeedAsk, Bool, String

### Community 103 - "ToolCall"
Cohesion: 0.43
Nodes (5): CockpitPresentation.Session, ShellCall, String, ToolResult, TranscriptEvent

### Community 104 - "ProjectFixture"
Cohesion: 0.27
Nodes (6): halfUnreadableRegistry(), ProjectRegistry, ProjectFixture, Bool, String, URL

### Community 105 - "CockpitRoom"
Cohesion: 0.13
Nodes (13): CockpitRoom, code, sessions, work, Self, RoomsVessel, RoomTab, Bool (+5 more)

### Community 106 - "FeedCallRun.swift"
Cohesion: 0.26
Nodes (6): FeedCall, FeedCall.Churn, FeedCall.Ending, FeedCallRun, Bool, FeedRow

### Community 107 - "SessionsDeck"
Cohesion: 0.18
Nodes (9): FeedFocus, lightbox, panel, row, FeedRow, FeedPreview, FeedRow, Int (+1 more)

### Community 108 - "SessionCommands"
Cohesion: 0.13
Nodes (14): CockpitView, Binding, ComposerDraft, ComposerSend, String, Void, SessionCommandItems, FocusedValues (+6 more)

### Community 109 - "ArgoColor"
Cohesion: 0.10
Nodes (18): GitHubDeviceChallenge, Response, declined, deviceCode, nonsense, pending, refused, revoked (+10 more)

### Community 110 - ".session"
Cohesion: 0.17
Nodes (11): RosterSessionFixture, Bool, CockpitPresentation, Int, String, rows(), CockpitPresentation, SessionRosterProjection (+3 more)

### Community 111 - "LaunchConfiguration"
Cohesion: 0.14
Nodes (16): FeedTableCoordinator, AnyView, Bool, CGFloat, EnvironmentValues, FeedRow, Int, Never (+8 more)

### Community 112 - "SessionAnnotationStore"
Cohesion: 0.35
Nodes (5): SessionAnnotationStore, Bool, SessionAnnotations, String, URL

### Community 113 - "ProcessLivenessReader"
Cohesion: 0.31
Nodes (5): ProcessLivenessReader, Bool, Set, ShellCommand, String

### Community 114 - "Hub"
Cohesion: 0.27
Nodes (4): Hub, Bool, SessionOwnership, String

### Community 115 - "ToolCallKind"
Cohesion: 0.15
Nodes (13): ToolCallKind, delegate, edit, execute, fetch, mcp, other, plan (+5 more)

### Community 116 - "AgentSpawnError"
Cohesion: 0.15
Nodes (12): AccountsCoordinator, AccountPort, AccountRegistryStore, Bool, ConnectNote, ConnectPort, Never, ProjectBinding (+4 more)

### Community 117 - "AgentProcessEvents"
Cohesion: 0.09
Nodes (20): AnyObject, AgentLaunch, String, AgentProcess, AgentProcessEvents, AgentProcessHost, AgentSpawnError, executableNotFound (+12 more)

### Community 118 - "ArgoGlyph"
Cohesion: 0.27
Nodes (5): FeedMarkLine, ArgoColor, String, ArgoGlyph, String

### Community 119 - "FeedMarkdownTable"
Cohesion: 0.31
Nodes (6): FeedMarkdownTable, ArgoColor, Font, Int, String, TableSlot

### Community 120 - "PlanPillE2ETests"
Cohesion: 0.27
Nodes (4): PlanPillE2ETests, Bool, String, XCUIElement

### Community 121 - "Codable"
Cohesion: 0.31
Nodes (5): SessionAnnotations, SessionAnnotations.Annotation, Decoder, Encoder, Codable

### Community 122 - ".spawnSession"
Cohesion: 0.31
Nodes (4): Hub, AgentCLI, Int32, SessionOwnership

### Community 123 - "AgentSpawn"
Cohesion: 0.39
Nodes (7): AgentSpawn, Exit, AgentCLI, Int, Int32, SessionOwnership, String

### Community 124 - "Project"
Cohesion: 0.36
Nodes (6): CockpitPresentation, Project, Bool, Int, String, Connection

### Community 125 - ".figure"
Cohesion: 0.38
Nodes (5): FeedSpend, Double, Int, String, Usage

### Community 126 - "RosterOrder"
Cohesion: 0.16
Nodes (10): RosterOrder, Bool, Row, String, ShellSidebar, Binding, Bool, CockpitPresentation (+2 more)

### Community 127 - "SessionRow"
Cohesion: 0.13
Nodes (10): AccountRecord, AccountProvider, String, AccountRegistry, AccountRegistry, LenientRecord, Decoder, Encoder (+2 more)

### Community 128 - "ProjectDrawerProjection"
Cohesion: 0.14
Nodes (12): ProjectDrawer, Bool, CockpitPresentation, ProjectDrawerProjection, Row, Bool, CockpitPresentation, Int (+4 more)

### Community 129 - "ArgoTheme"
Cohesion: 0.36
Nodes (5): ArgoTheme, EnvironmentValues, ArgoPalette, View, ColorScheme

### Community 130 - "HandoffWait"
Cohesion: 0.42
Nodes (5): HandoffPatience, HandoffWait, Int, Void, HandoffTestClock

### Community 131 - ".briefURL"
Cohesion: 0.36
Nodes (4): HandoffScript, Int, String, URL

### Community 132 - "ProjectRegistry+Codable.swift"
Cohesion: 0.20
Nodes (8): CodingKeys, activeProjectID, projects, LenientRecord, ProjectRegistry, Decoder, Encoder, ProjectRecord

### Community 133 - "SessionLiveness"
Cohesion: 0.13
Nodes (14): SessionLiveness, live, quiet, SessionProvenance, external, managed, orphaned, Bool (+6 more)

### Community 134 - "StopReason"
Cohesion: 0.22
Nodes (7): StopReason, cancelled, endTurn, maxTokens, maxTurnRequests, refusal, unknown

### Community 135 - ".promptEvents"
Cohesion: 0.61
Nodes (7): text, commandPrompt(), firstText(), localCommandOutput(), String, tag(), userPrompt()

### Community 136 - "diffEvidence"
Cohesion: 0.32
Nodes (11): raw, count(), declaredChange(), declaredDestination(), diffEvidence(), diffLine(), fileChange(), hunk() (+3 more)

### Community 137 - "firstLines"
Cohesion: 0.14
Nodes (16): PermissionRequest, read(), String, PermissionPrompt, String, Void, PermissionPromptProjection, Prompt (+8 more)

### Community 138 - "CockpitView"
Cohesion: 0.18
Nodes (10): CockpitView, Binding, Bool, CockpitPresentation, FeedRow, SessionHeaderProjection, String, TranscriptEvent (+2 more)

### Community 139 - "DeckSeam"
Cohesion: 0.28
Nodes (7): DeckSeam, EnvironmentValues, Bool, CGFloat, ClosedRange, Void, Gesture

### Community 140 - "EvidenceHunk"
Cohesion: 0.31
Nodes (8): EvidenceDiffLine, EvidenceHunk, EvidenceHunkProse, EvidenceUnreadablePatch, ArgoColor, AttributedString, Int, String

### Community 141 - "EvidenceHeader"
Cohesion: 0.12
Nodes (17): BindingProbe, BindingScopeCheck, ProviderScopeCheck, ScopeVisibility, notVisible, unauthorized, unreadable, visible (+9 more)

### Community 142 - "FeedCallSubject"
Cohesion: 0.17
Nodes (9): InMemorySessionDriver, SessionAttachment, String, URL, UUID, PermissionDecision, allow, allowAlways (+1 more)

### Community 143 - "FeedQuietCommand"
Cohesion: 0.36
Nodes (5): FeedQuietCommand, Bool, Int, Set, String

### Community 144 - "FeedSurvey"
Cohesion: 0.16
Nodes (15): FeedCall, FeedSurvey, Bool, FeedCall, FeedEvidence, Int, String, Tally (+7 more)

### Community 145 - "FeedProjection"
Cohesion: 0.33
Nodes (5): FeedProjection, [FeedRow], Sequence<FeedRow>, FeedRow, Int

### Community 146 - "FeedRowButtonStyle"
Cohesion: 0.18
Nodes (9): FeedRowButtonStyle, ArgoColor, Bool, Configuration, View, ButtonStyle, QuietButtonStyle, Configuration (+1 more)

### Community 147 - "FeedTail"
Cohesion: 0.29
Nodes (5): FeedTail, Bool, CGFloat, FeedRow, Int

### Community 148 - ".step"
Cohesion: 0.12
Nodes (15): CockpitPresentation, EnvironmentValues, FeedHandoff, String, Void, FeedMark, compacted, handedOff (+7 more)

### Community 149 - "PlanPill"
Cohesion: 0.31
Nodes (8): PlanCounter, PlanPill, PlanRing, Reading, ArgoColor, Bool, Double, String

### Community 150 - ".rename"
Cohesion: 0.53
Nodes (3): SessionTitle, CockpitPresentation, String

### Community 151 - "CockpitCoordinator"
Cohesion: 0.36
Nodes (3): CockpitCoordinator, Int, String

### Community 152 - "CodingKeys"
Cohesion: 0.29
Nodes (7): CodingKeys, accounts, CodingKeys, archived, name, sessions, CodingKey

### Community 153 - "CompanionChannel"
Cohesion: 0.33
Nodes (5): CompanionChannel, SessionOwnership, String, URL, Void

### Community 154 - ".materialize"
Cohesion: 0.47
Nodes (4): CompanionPlugin, SessionOwnership, String, URL

### Community 155 - "Data"
Cohesion: 0.23
Nodes (10): ConnectPanelMode, creating, settings, AgentCLI, ConnectPanelProjection, Panel, Row, Bool (+2 more)

### Community 156 - "HubObservation"
Cohesion: 0.39
Nodes (6): HubObservation, State, live, stopped, String, URL

### Community 157 - "CheckoutReader"
Cohesion: 0.36
Nodes (4): CheckoutReader, GitCommand, String, URL

### Community 158 - "SessionProvenance"
Cohesion: 0.21
Nodes (12): AccountChoice, ConnectPanelProjection, Offer, PortRow, AccountPort, AccountProvider, BindingFault, Bool (+4 more)

### Community 159 - ".lines"
Cohesion: 0.22
Nodes (8): AttributedString, HighlightColors, Int, SyntaxPatch, coloured(), parsed(), AttributedString, String

### Community 160 - "AnnotationFile"
Cohesion: 0.29
Nodes (4): AnnotationFile, SessionAnnotationStore, String, URL

### Community 161 - "rosterOf"
Cohesion: 0.20
Nodes (15): DeviceCodeResponse, PollOutcome, pending, slowDown, Duration, Int, String, TokenErrorResponse (+7 more)

### Community 162 - ".showed"
Cohesion: 0.12
Nodes (14): SessionDriveError, attachmentUnwritable, cannotAttach, noSuchGrant, notDrivable, nothingPending, nothingToSend, SessionDriver (+6 more)

### Community 163 - "DeckZone"
Cohesion: 0.20
Nodes (8): DeckSlot, DeckZone, minimap, rail, tabs, Bool, Self, String

### Community 164 - "FeedAgents"
Cohesion: 0.36
Nodes (4): FeedAgents, FeedCall, FeedRow, Int

### Community 165 - "FeedProse"
Cohesion: 0.36
Nodes (7): EnvironmentValues, FeedProse, ArgoColor, String, Voice, message, thought

### Community 166 - "InstrumentDeckShell"
Cohesion: 0.17
Nodes (11): InstrumentDeckShell, Binding, CGFloat, CockpitPresentation, ComposerDraft, ComposerSend, FeedRow, Int (+3 more)

### Community 167 - ".labels"
Cohesion: 0.50
Nodes (3): DistinguishingLabel, Set, String

### Community 170 - "RosterOrderE2ETests"
Cohesion: 0.38
Nodes (4): RosterOrderE2ETests, Bool, String, XCUIElement

### Community 171 - "describe"
Cohesion: 0.38
Nodes (6): describe(), oneLine(), Int, String, ToolResult, TranscriptEvent

### Community 172 - "HubConnection"
Cohesion: 0.13
Nodes (12): HubConnection, connected, connecting, failed, idle, String, ConnectionHealthProjection, ConnectionChip (+4 more)

### Community 173 - "TestGate"
Cohesion: 0.29
Nodes (4): Never, Void, TestGate, CheckedContinuation

### Community 174 - "CockpitPresentation.Session"
Cohesion: 0.20
Nodes (9): LiveClaudeFixture, RecordingProcessHost, Bool, Hub, Int, SessionOwnership, String, UInt8 (+1 more)

### Community 175 - ".folded"
Cohesion: 0.52
Nodes (3): FeedSurveyFold, FeedCall, FeedRow

### Community 176 - ".pin"
Cohesion: 0.12
Nodes (13): AccountPort, codeHost, workItem, AccountProvider, github, linear, ConnectionCause, offline (+5 more)

### Community 177 - "FeedUnreadable"
Cohesion: 0.22
Nodes (7): FeedUnreadable, FeedUnreadableRun, FeedRow, Int, String, FeedUnreadableLine, Bool

### Community 178 - "SessionHeader"
Cohesion: 0.32
Nodes (5): SessionHeader, SessionHeaderGallery, CGFloat, SessionHeaderProjection, Void

### Community 179 - "NewSessionOffer"
Cohesion: 0.43
Nodes (4): NewSessionOffer, Bool, CockpitPresentation, String

### Community 180 - "SessionSpendFixture"
Cohesion: 0.38
Nodes (5): Rhythm, SessionSpendFixture, Int, SessionHeaderProjection, TranscriptEvent

### Community 181 - "FeedHandoffTests.swift"
Cohesion: 0.33
Nodes (6): FeedRow.Content, presentation(), session(), Bool, CockpitPresentation, String

### Community 182 - "ArgoApp"
Cohesion: 0.38
Nodes (6): AccountsCoordinator, App, ArgoApp, String, CockpitCoordinator, Scene

### Community 184 - "FeedSwitchE2ETests"
Cohesion: 0.47
Nodes (3): FeedSwitchE2ETests, String, XCUIElement

### Community 186 - "Entry"
Cohesion: 0.60
Nodes (3): Entry, HubJoinCache, String

### Community 187 - "LaunchProject"
Cohesion: 0.15
Nodes (12): GitHubDeviceChallenge, GitHubDeviceFlowError, declined, expired, malformedResponse, refused, Duration, String (+4 more)

### Community 188 - ".read"
Cohesion: 0.53
Nodes (3): LoginShellPath, ShellCommand, String

### Community 189 - "send"
Cohesion: 0.33
Nodes (6): relay(), send(), Any, SessionOwnership, String, Process

### Community 190 - "ComposerMode"
Cohesion: 0.13
Nodes (12): ComposerFooter, Bool, SessionAttachment, String, Void, ComposerMode, ask, code (+4 more)

### Community 191 - ".call"
Cohesion: 0.33
Nodes (4): EvidenceFixture, Bool, FeedCall, FeedEvidence

### Community 192 - "FeedAskOption"
Cohesion: 0.47
Nodes (5): FeedAskOption, FeedAskOptions, ArgoColor, Bool, String

### Community 193 - "FeedCallLine"
Cohesion: 0.33
Nodes (5): FeedCallLine, ArgoColor, Bool, FeedCall, Void

### Community 194 - "FeedView"
Cohesion: 0.33
Nodes (3): FeedView, Int, View

### Community 195 - "FeedShotView"
Cohesion: 0.33
Nodes (5): FeedShotView, Bool, CGFloat, String, Void

### Community 196 - "ContextBar"
Cohesion: 0.18
Nodes (13): BindingHealth, ConnectionState, healthy, needsReconnect, stale, ConnectionCause, ConnectionHealthReading, PortConnection (+5 more)

### Community 197 - "SessionHeaderProjection.Context.Tier"
Cohesion: 0.53
Nodes (3): SessionHeaderProjection.Context.Tier, ArgoColor, ArgoPalette

### Community 198 - "RosterSearch"
Cohesion: 0.60
Nodes (3): RosterSearch, SessionRosterProjection, String

### Community 199 - ".swatches"
Cohesion: 0.16
Nodes (10): Void, ComposerPreview, SessionComposer, Binding, Bool, ComposerDraft, ComposerSend, SessionAttachment (+2 more)

### Community 200 - "FeedNewMessagesTests.swift"
Cohesion: 0.40
Nodes (5): Leaving, numbered(), FeedRow, Int, String

### Community 201 - "ProjectProjectionTests.swift"
Cohesion: 0.47
Nodes (5): observe(), presentation(), CockpitPresentation, Hub, String

### Community 202 - "SessionHeaderContextTests.swift"
Cohesion: 0.40
Nodes (5): session(), CockpitPresentation, Int, SessionHeaderProjection, tier()

### Community 203 - "header"
Cohesion: 0.33
Nodes (5): header(), AgentCLI, CockpitPresentation, SessionHeaderProjection, String

### Community 204 - "header"
Cohesion: 0.33
Nodes (5): header(), CockpitPresentation, Int, SessionHeaderProjection, String

### Community 207 - ".beginDiscovery"
Cohesion: 0.29
Nodes (9): granted, StandingAllow, StandingAllowTable, Bool, SessionOwnership, String, session(), CockpitPresentation (+1 more)

### Community 208 - "Tier"
Cohesion: 0.40
Nodes (4): Tier, convention, derived, direct

### Community 209 - "AgentLaunch"
Cohesion: 0.28
Nodes (8): ConnectionHealthLedger, Key, Reading, ConnectionCause, ProjectBinding, Set, String, Reading

### Community 210 - "SessionAnnotationStoreTests.swift"
Cohesion: 0.40
Nodes (3): SessionAnnotationStore, Bool, String

### Community 211 - "CockpitPresentation.Session"
Cohesion: 0.40
Nodes (3): CockpitPresentation.Session, String, TranscriptEvent

### Community 212 - "ComposerFooter"
Cohesion: 0.17
Nodes (10): FeedTableCoordinator, Bool, CGFloat, FeedRow, Int, NSTableView, NSTableColumn, NSTableViewDataSource (+2 more)

### Community 213 - "SendButton"
Cohesion: 0.40
Nodes (4): SendButton, ArgoColor, Bool, Void

### Community 214 - "FeedAskQuestion"
Cohesion: 0.25
Nodes (6): FeedTableCoordinator, Bool, CGFloat, Int, TimeInterval, Notification

### Community 215 - ".latch"
Cohesion: 0.28
Nodes (7): CompanionSocket, Bool, DispatchSourceRead, Int, Int32, String, Void

### Community 216 - ".move"
Cohesion: 0.19
Nodes (9): SessionTitle, Standing, explicit, placeholder, prompt, provisional, Bool, String (+1 more)

### Community 217 - "PlanStepLine"
Cohesion: 0.14
Nodes (12): PlanProjection, TranscriptEvent, PlanReading, Step, Double, Int, String, PlanStepLine (+4 more)

### Community 218 - ".phrase"
Cohesion: 0.30
Nodes (7): KeychainGrantError, keychain, KeychainGrantStore, Any, String, OSStatus, Security

### Community 219 - "NewSessionButton"
Cohesion: 0.29
Nodes (5): NewSessionButton, ArgoColor, CGFloat, String, Void

### Community 220 - "ChurningRosterSpecimen"
Cohesion: 0.40
Nodes (3): ChurningRosterSpecimen, CockpitPresentation, Int

### Community 221 - "FoundationSpecimen"
Cohesion: 0.40
Nodes (3): FoundationSpecimen, CockpitPresentation, ToolbarContent

### Community 222 - "SessionHeaderSpecimen"
Cohesion: 0.50
Nodes (3): SessionHeaderSpecimen, CockpitPresentation, SessionHeaderProjection

### Community 223 - "ArgoFeedRow.swift"
Cohesion: 0.40
Nodes (3): ArgoFeedRow, CGFloat, View

### Community 224 - "EvidenceAddressTests.swift"
Cohesion: 0.60
Nodes (4): call(), execute(), FeedCall, String

### Community 225 - "MarkedProseTests.swift"
Cohesion: 0.18
Nodes (10): BindingResolution, broken, ready, unbound, ResolvedBinding, AccountProvider, BindingFault, ProjectBinding (+2 more)

### Community 226 - "RosterSearchTests.swift"
Cohesion: 0.40
Nodes (4): matching(), RosterSearchTests, SessionRosterProjection, String

### Community 227 - "SessionRenameProjectionTests.swift"
Cohesion: 0.40
Nodes (4): session(), Bool, CockpitPresentation, String

### Community 228 - "Group"
Cohesion: 0.39
Nodes (5): Family, Group, Any, ArgoColor, String

### Community 229 - "CockpitCoordinator+Send.swift"
Cohesion: 0.31
Nodes (4): CockpitCoordinator, Bool, SessionAttachment, String

### Community 232 - "AgentLauncherTests.swift"
Cohesion: 0.50
Nodes (3): directoryHolding(), String, URL

### Community 233 - "CheckoutReaderTests.swift"
Cohesion: 0.50
Nodes (3): gitAnswering(), GitCommand, String

### Community 234 - "HandoffChainTests.swift"
Cohesion: 0.50
Nodes (3): handedOffTo(), Hub, String

### Community 235 - "answer"
Cohesion: 0.24
Nodes (6): PermissionRequest.Target, Self, String, Target, command, edit

### Community 236 - "HubHandoffTests.swift"
Cohesion: 0.50
Nodes (3): handedOffTo(), Hub, String

### Community 237 - "ProcessLivenessReaderTests.swift"
Cohesion: 0.50
Nodes (3): shell(), ShellCommand, String

### Community 238 - "SessionOwnershipTests.swift"
Cohesion: 0.67
Nodes (3): Clock, registry(), SessionOwnership

### Community 239 - "ComposerField"
Cohesion: 0.50
Nodes (3): ComposerField, String, Void

### Community 240 - "ComposerSeam"
Cohesion: 0.40
Nodes (4): ComposerSeam, ArgoColor, String, Void

### Community 241 - "Hashable"
Cohesion: 0.25
Nodes (10): SessionAttachment, Source, bytes, file, Bool, Data, Int, String (+2 more)

### Community 242 - "FeedTailButton"
Cohesion: 0.50
Nodes (3): FeedTailButton, String, Void

### Community 243 - "SessionHeaderContext"
Cohesion: 0.33
Nodes (4): SessionHeaderContext, ArgoColor, Bool, SessionHeaderProjection

### Community 244 - "ProjectVessel"
Cohesion: 0.21
Nodes (8): CockpitPresentation, CockpitPresentation.Session, CockpitPresentation.Session.Access, CockpitPresentation.Session.Workspace, Bool, Hub, HubSession, SessionAnnotations

### Community 245 - "SpawningRosterSpecimen"
Cohesion: 0.22
Nodes (9): ConnectCompanion, includedWithSpawns, unknown, ConnectReading, AccountPort, AccountProvider, ConnectNote, ConnectPort (+1 more)

### Community 246 - "ArgoBadge"
Cohesion: 0.22
Nodes (9): ConnectPanel, ConnectPanelProjection, ConnectPanelActions, AccountPort, AccountProvider, ProjectBinding, Void, ConnectSheet (+1 more)

### Community 247 - "ArgoOpacity"
Cohesion: 0.50
Nodes (3): ArgoOpacity, Double, String

### Community 249 - "FeedScaleTests.swift"
Cohesion: 0.50
Nodes (3): elapsed(), Duration, Void

### Community 250 - "session"
Cohesion: 0.50
Nodes (4): session(), AgentCLI, CockpitPresentation, String

### Community 251 - "SessionHeaderProjectionTests.swift"
Cohesion: 0.67
Nodes (3): session(), CockpitPresentation, String

### Community 253 - ".liveCwds"
Cohesion: 0.31
Nodes (7): AttributedString, Bool, HighlightColors, String, SyntaxHighlight, highlighted(), String

### Community 258 - "AgentChip"
Cohesion: 0.21
Nodes (10): FeedTableModel, AnyView, Binding, Bool, CGFloat, EnvironmentValues, FeedRow, Int (+2 more)

### Community 259 - "FeedGalleryRow"
Cohesion: 0.18
Nodes (9): CodingKeys, bindings, id, path, LenientBinding, ProjectRecord, Decoder, Encoder (+1 more)

### Community 260 - "Usage"
Cohesion: 0.18
Nodes (8): AttachmentDropTarget, Bool, SessionAttachment, View, Void, ComposerDraft, Bool, SessionAttachment

### Community 262 - "ArgoMarkedName"
Cohesion: 0.26
Nodes (12): EvidenceAddress, filed, named, typed, FeedCall, FeedEvidence, Step, Bool (+4 more)

### Community 263 - "presentation"
Cohesion: 0.67
Nodes (3): presentation(), CockpitPresentation, String

### Community 269 - "ClaudeSessionDriver"
Cohesion: 0.23
Nodes (6): ClaudeSessionDriver, Bool, SessionAttachment, SessionOwnership, String, URL

### Community 270 - "ProjectRecord"
Cohesion: 0.26
Nodes (6): ProjectRecord, AccountPort, Bool, ProjectBinding, String, URL

### Community 271 - "ComposerDraft"
Cohesion: 0.39
Nodes (6): ComposerDraft, Bool, ComposerSend, Int, SessionAttachment, String

### Community 272 - ".attachments"
Cohesion: 0.23
Nodes (8): ComposerPasteboard, Data, NSPasteboard, SessionAttachment, AttachmentFixture, Data, SessionAttachment, UTType

### Community 273 - "PermissionPromptFooter.swift"
Cohesion: 0.26
Nodes (11): Answer, allow, deny, PermissionDecisionButton, PermissionKeycap, PermissionPromptFooter, StandingAllowOffer, ArgoColor (+3 more)

### Community 274 - "WrapFlow"
Cohesion: 0.33
Nodes (7): CGFloat, CGRect, CGSize, WrapFlow, Layout, ProposedViewSize, Subviews

### Community 275 - "ArgoFloatingGlass"
Cohesion: 0.23
Nodes (8): ArgoFloatingGlass, EnvironmentValues, ArgoColor, Bool, View, View, InsettableShape, Vessel

### Community 276 - "ConnectionFault"
Cohesion: 0.20
Nodes (8): BindingFault, ConnectionFault, grantRefused, read, ConnectionLevel, account, binding, ConnectionCause

### Community 277 - "RecordDirectoryFixture"
Cohesion: 0.33
Nodes (5): FixtureTranscript, RecordDirectoryFixture, String, TimeInterval, URL

### Community 278 - "ComposerSeamNote"
Cohesion: 0.24
Nodes (8): ComposerSeamNote, draftKept, notice, refusal, ComposerDraft, Int, Self, String

### Community 279 - "AccountBindingReference"
Cohesion: 0.31
Nodes (6): AccountBindingReference, AccountRemoval, AccountPort, AccountRegistry, String, String

### Community 280 - "AttachmentStore"
Cohesion: 0.31
Nodes (6): AttachmentStore, SessionAttachment, Bool, SessionAttachment, String, URL

### Community 281 - "PermissionExpiry"
Cohesion: 0.24
Nodes (5): PermissionExpiry, String, Hub, SessionOwnership, FeedRow.Content

### Community 282 - "ConnectChallenge"
Cohesion: 0.27
Nodes (8): ConnectChallenge, AccountProvider, String, URL, DeviceCodeCard, String, Void, ConnectFixture

### Community 283 - "ConnectPortState"
Cohesion: 0.29
Nodes (8): ConnectPort, ConnectPortState, bound, broken, unbound, AccountPort, BindingFault, String

### Community 284 - ".decode"
Cohesion: 0.29
Nodes (6): AttachmentThumbnail, Image, SessionAttachment, CGImage, CGImageSource, ImageIO

### Community 285 - "ComposerDrafts"
Cohesion: 0.38
Nodes (6): ComposerDrafts, Binding, Bool, ComposerDraft, Int, String

### Community 286 - "SessionsDeck"
Cohesion: 0.20
Nodes (9): SessionsDeck, Binding, ComposerDraft, ComposerSend, FeedRow, Int, SessionHeaderProjection, String (+1 more)

### Community 288 - "AttachedTerminal"
Cohesion: 0.25
Nodes (4): AttachedTerminal, Int, String, Void

### Community 289 - "ConnectPortRow"
Cohesion: 0.44
Nodes (5): ConnectPortRow, Draft, Binding, ConnectPanelProjection, String

### Community 290 - "QueuedTurn"
Cohesion: 0.31
Nodes (6): QueuedTurn, SessionAttachment, String, UUID, QueuedTurnChip, Void

### Community 291 - "EvidenceListing"
Cohesion: 0.42
Nodes (5): EvidenceListing, Line, Bool, Int, String

### Community 292 - "FeedTableView"
Cohesion: 0.31
Nodes (6): FeedTableView, Bool, Int, Void, NSEvent, NSTableView

### Community 293 - ".sessions"
Cohesion: 0.39
Nodes (5): SpecimenScreen, FeedRow, Int, SessionHeaderProjection, View

### Community 294 - ".connection"
Cohesion: 0.36
Nodes (4): AccountsCoordinator, AccountRegistry, ConnectPort, ProjectRecord

### Community 295 - "GitHubAuthorization"
Cohesion: 0.36
Nodes (4): GitHubAuthorization, AccountRegistryStore, GitHubDeviceChallenge, GitHubDeviceFlow

### Community 296 - "resolvedPath"
Cohesion: 0.25
Nodes (5): resolvedPath(), String, HubSession, Bool, Int

### Community 297 - "FeedTableDelta"
Cohesion: 0.25
Nodes (6): FeedTableDelta, append, reload, FeedRow, Int, Range

### Community 298 - ".contains"
Cohesion: 0.43
Nodes (4): ProjectScope, Bool, String, URL

### Community 299 - "PermissionPromptTarget"
Cohesion: 0.33
Nodes (4): PermissionPromptHunk, PermissionPromptTarget, CGFloat, String

### Community 300 - ".allows"
Cohesion: 0.33
Nodes (3): StandingAllowProjection, CockpitPresentation, String

### Community 301 - "FeedTableHandle"
Cohesion: 0.29
Nodes (4): FeedTableHandle, FeedRow, FeedTableCoordinator, TimeInterval

### Community 302 - "SessionState"
Cohesion: 0.48
Nodes (3): Reading, SessionState, String

### Community 303 - "ToolbarSegment"
Cohesion: 0.43
Nodes (4): Bool, View, ToolbarSegment, View

### Community 304 - "ProjectSettingsCommands"
Cohesion: 0.33
Nodes (4): ProjectSettingsCommands, CockpitPresentation, String, Void

### Community 305 - "AttachButton.swift"
Cohesion: 0.33
Nodes (4): AttachButton, SessionAttachment, Void, UniformTypeIdentifiers

### Community 306 - "AttachmentProjection"
Cohesion: 0.60
Nodes (3): AttachmentProjection, SessionAttachment, String

### Community 307 - "EvidenceSource"
Cohesion: 0.40
Nodes (4): EvidenceSource, EvidenceSourceLine, AttributedString, String

### Community 308 - "FeedRowCell"
Cohesion: 0.47
Nodes (5): FeedRowCell, AnyView, NSCoder, NSHostingView, NSTableCellView

### Community 309 - ".makeScrollView"
Cohesion: 0.40
Nodes (3): FeedTableCoordinator, NSScrollView, NSScrollView

### Community 310 - ".rename"
Cohesion: 0.33
Nodes (3): Rename, CockpitPresentation, String

### Community 311 - "AccountProvider"
Cohesion: 0.50
Nodes (3): AccountProvider, AccountPort, Bool

### Community 312 - "ProjectBinding"
Cohesion: 0.60
Nodes (3): ProjectBinding, AccountPort, String

### Community 314 - ".phrase"
Cohesion: 0.40
Nodes (3): AgePhrase, Int, String

### Community 315 - ".heading"
Cohesion: 0.40
Nodes (3): DeviceCodeCopy, AccountProvider, String

### Community 316 - "AttachmentChip"
Cohesion: 0.40
Nodes (4): AttachmentChip, Image, SessionAttachment, Void

### Community 317 - "StandingAllowTray"
Cohesion: 0.60
Nodes (4): StandingAllowChip, StandingAllowTray, String, Void

### Community 318 - ".isWorking"
Cohesion: 0.40
Nodes (3): FeedWorking, Bool, CockpitPresentation

### Community 319 - "FeedLightbox"
Cohesion: 0.40
Nodes (4): FeedLightbox, String, Void, View

### Community 320 - "FeedMarkdownFence"
Cohesion: 0.40
Nodes (3): FeedMarkdownFence, AttributedString, String

### Community 321 - "SessionHandoffButton"
Cohesion: 0.40
Nodes (4): SessionHandoffButton, ArgoColor, SessionHeaderProjection, Void

### Community 322 - "GuideLine"
Cohesion: 0.50
Nodes (4): GuideLine, SessionHeaderProjection.Context, String, Tier

### Community 323 - "EvidenceStepTests.swift"
Cohesion: 0.40
Nodes (4): edited(), FeedCall, Int, String

### Community 325 - "LivePermissionTests.swift"
Cohesion: 0.67
Nodes (3): command(), prompt(), String

### Community 327 - "ConnectRow"
Cohesion: 0.50
Nodes (3): ConnectRow, ConnectPanelProjection, Trailing

### Community 328 - "Benefit"
Cohesion: 0.50
Nodes (3): Benefit, String, WelcomeCopy

### Community 329 - "AttachmentTray"
Cohesion: 0.50
Nodes (3): AttachmentTray, SessionAttachment, Void

### Community 331 - "ConnectCopyTests.swift"
Cohesion: 0.50
Nodes (3): strings(), ConnectPanelProjection, String

### Community 332 - "Row"
Cohesion: 0.50
Nodes (4): Row, Bool, CGFloat, String

### Community 341 - "session"
Cohesion: 0.67
Nodes (3): session(), CockpitPresentation, TranscriptEvent

## Knowledge Gaps
- **505 isolated node(s):** `SessionRenameStrings`, `nonExpiring`, `expiring`, `workItem`, `codeHost` (+500 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **34 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `ArgoEngine` connect `Project Strip UI` to `Transcript Event Description & Errors`, `Diff Evidence Model`, `Agent Plan & Tool Reading`, `Cockpit Shell View Scaffolding`, `Sessions Deck UI`, `Elevation & Shadow Tokens`, `Testing`, `Session`, `MediaProvenance`, `.call`, `FeedCall`, `Equatable`, `.rows`, `CockpitCoordinator.swift`, `.spend`, `SessionComposerProjection`, `EvidenceReading`, `HubTestObservation.swift`, `FeedCallReading`, `ToolCallOutcome`, `CockpitActions`, `CockpitPresentation.Session`, `.header`, `DeckContentRow`, `Identifiable`, `FeedShot`, `FeedView`, `Ask`, `ToolCall`, `FeedCallRun.swift`, `SessionCommands`, `AgentProcessEvents`, `Project`, `.figure`, `firstLines`, `CockpitView`, `EvidenceHunk`, `FeedProjection`, `.step`, `CockpitCoordinator`, `Data`, `SessionProvenance`, `FeedAgents`, `InstrumentDeckShell`, `describe`, `HubConnection`, `NewSessionOffer`, `SessionSpendFixture`, `FeedHandoffTests.swift`, `LaunchProjectTests.swift`, `LaunchProject`, `ComposerMode`, `FeedCallLine`, `FeedShotView`, `ContextBar`, `.swatches`, `ProjectProjectionTests.swift`, `header`, `header`, `SessionAnnotationStoreTests.swift`, `CockpitPresentation.Session`, `PlanStepLine`, `MarkedProseTests.swift`, `RosterSearchTests.swift`, `SessionRenameProjectionTests.swift`, `CockpitCoordinator+Send.swift`, `AgentLauncherTests.swift`, `CheckoutReaderTests.swift`, `HandoffChainTests.swift`, `HubHandoffTests.swift`, `ProcessLivenessReaderTests.swift`, `SessionOwnershipTests.swift`, `ComposerSeam`, `ProjectVessel`, `SpawningRosterSpecimen`, `EvidenceSyntaxTests.swift`, `SessionSpendTests.swift`, `AgentCLI+ReadableName.swift`, `CockpitPresentation+PreviewFanout.swift`, `CockpitPresentation+PreviewTranscript.swift`, `Usage`, `ArgoMarkedName`, `SessionRosterArchiveTests.swift`, `SessionRosterProjectionTests.swift`, `NarrationReadingTests.swift`, `PermissionPromptFooter.swift`, `ConnectPortState`, `.decode`, `SessionsDeck`, `ConnectPortRow`, `PermissionPromptTarget`, `.allows`, `SessionState`, `AttachButton.swift`, `.heading`, `AttachmentChip`, `StandingAllowTray`, `.isWorking`, `EvidenceStepTests.swift`, `SessionAttachmentTests.swift`, `LivePermissionTests.swift`, `AttachmentTray`, `ConnectCopyTests.swift`, `HubSessionEventsTests.swift`, `LaunchProjectTests.swift`, `SessionContextTests.swift`, `AccountPort+ReadableName.swift`, `AccountProvider+ReadableName.swift`, `ConnectionCause+ReadableName.swift`, `ComposerPasteboardTests.swift`, `EvidenceLanguageTests.swift`?**
  _High betweenness centrality (0.159) - this node is a cross-community bridge._
- **Why does `Foundation` connect `Project Strip UI` to `App Startup & Transcript Engine`, `Transcript Content Blocks`, `Transcript Record Parsing`, `Transcript Event Description & Errors`, `Cockpit Rooms & Deck Zones`, `File Tailing & Watching`, `Agent Plan & Tool Reading`, `Git Checkout Projection`, `Cockpit Shell View Scaffolding`, `Elevation & Shadow Tokens`, `Connection Status Chip`, `Shell Sidebar UI`, `Testing`, `AgentTerminals`, `MarkdownBlock`, `SessionDiscovery`, `CompanionConnection`, `ClaimID`, `observe`, `HubJoin`, `WorkspaceProjection`, `String`, `.rows`, `CockpitCoordinator.swift`, `.spend`, `.invite`, `Context`, `Sendable`, `ChangeSink`, `ResolvedCall`, `HubTestObservation.swift`, `FeedCallReading`, `FeedFixture`, `.run`, `CockpitCoordinator`, `ProjectRegistryStore`, `AgentLauncher`, `HandoffChain`, `HubSession`, `SpawnFixture`, `SessionAnnotations`, `CheckoutFixture`, `CompanionFact`, `Engine`, `ProjectRecord`, `TranscriptObservation`, `CompanionOutcome`, `FeedView`, `SessionAnnotationStore`, `Hub`, `AgentProcessEvents`, `Codable`, `.spawnSession`, `AgentSpawn`, `SessionRow`, `ProjectDrawerProjection`, `HandoffWait`, `.briefURL`, `ProjectRegistry+Codable.swift`, `.promptEvents`, `EvidenceHeader`, `FeedCallSubject`, `FeedQuietCommand`, `CompanionChannel`, `.materialize`, `HubObservation`, `CheckoutReader`, `rosterOf`, `.showed`, `.pin`, `LaunchProjectTests.swift`, `Hub`, `Entry`, `LaunchProject`, `.read`, `ContextBar`, `RosterSearch`, `ProjectProjectionTests.swift`, `.beginDiscovery`, `AgentLaunch`, `SessionAnnotationStoreTests.swift`, `.latch`, `.move`, `.phrase`, `EvidenceAddressTests.swift`, `MarkedProseTests.swift`, `RosterSearchTests.swift`, `AgentLauncherTests.swift`, `CheckoutReaderTests.swift`, `HandoffChainTests.swift`, `answer`, `HubHandoffTests.swift`, `Hashable`, `EvidenceSyntaxTests.swift`, `FeedScaleTests.swift`, `SessionSpendTests.swift`, `FeedGalleryRow`, `SessionRosterArchiveTests.swift`, `SessionRosterProjectionTests.swift`, `ClaudeSessionDriver`, `ProjectRecord`, `ConnectionFault`, `AccountBindingReference`, `AttachmentStore`, `PermissionExpiry`, `AttachedTerminal`, `EvidenceListing`, `GitHubAuthorization`, `resolvedPath`, `FeedTableDelta`, `.contains`, `AccountProvider`, `ProjectBinding`, `.keystrokes`, `SessionAttachmentTests.swift`, `LivePermissionTests.swift`, `WallClock.swift`, `HubSessionEventsTests.swift`, `LaunchProjectTests.swift`, `SessionContextTests.swift`?**
  _High betweenness centrality (0.157) - this node is a cross-community bridge._
- **Why does `SwiftUI` connect `Cockpit Shell View Scaffolding` to `Hub Connection & Usage State`, `Motion & Animation Tokens`, `Typography Tokens`, `Cockpit View & Actions`, `Sessions Deck UI`, `Specimen Deck Fixtures`, `Contract Specimen`, `Geometry & Spacing Tokens`, `Git Status Vessel`, `Project Strip UI`, `Session Navigator UI`, `Layout Constants`, `Testing`, `MarkdownBlock`, `MediaProvenance`, `ProseLinkRun`, `FeedRowSelection`, `CockpitCoordinator.swift`, `FeedProseText`, `SessionComposerProjection`, `EvidenceReading`, `ChangeSink`, `ArgoTypeScale`, `CockpitActions`, `ArgoIconSize`, `DeckContentRow`, `Identifiable`, `FeedShot`, `Position`, `FeedView`, `CockpitRoom`, `SessionsDeck`, `SessionCommands`, `ArgoGlyph`, `FeedMarkdownTable`, `RosterOrder`, `ProjectDrawerProjection`, `ArgoTheme`, `firstLines`, `CockpitView`, `DeckSeam`, `EvidenceHunk`, `FeedSurvey`, `FeedRowButtonStyle`, `FeedTail`, `.step`, `PlanPill`, `DeckZone`, `FeedProse`, `InstrumentDeckShell`, `HubConnection`, `FeedUnreadable`, `SessionHeader`, `ComposerMode`, `FeedAskOption`, `FeedCallLine`, `FeedView`, `FeedShotView`, `SessionHeaderProjection.Context.Tier`, `.swatches`, `SendButton`, `PlanStepLine`, `NewSessionButton`, `ChurningRosterSpecimen`, `FoundationSpecimen`, `SessionHeaderSpecimen`, `ArgoFeedRow.swift`, `ComposerField`, `ComposerSeam`, `FeedTailButton`, `SessionHeaderContext`, `ArgoBadge`, `EvidenceSyntaxTests.swift`, `AgentChip`, `Usage`, `SpecimenScreen`, `PermissionPromptFooter.swift`, `WrapFlow`, `ArgoFloatingGlass`, `.decode`, `SessionsDeck`, `ConnectPortRow`, `QueuedTurn`, `.sessions`, `PermissionPromptTarget`, `FeedTableHandle`, `ToolbarSegment`, `ProjectSettingsCommands`, `AttachButton.swift`, `EvidenceSource`, `AttachmentChip`, `StandingAllowTray`, `FeedLightbox`, `FeedMarkdownFence`, `SessionHandoffButton`, `ConnectRow`, `AttachmentTray`?**
  _High betweenness centrality (0.093) - this node is a cross-community bridge._
- **What connects `SessionRenameStrings`, `nonExpiring`, `expiring` to the rest of the system?**
  _505 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `App Startup & Transcript Engine` be split into smaller, more focused modules?**
  _Cohesion score 0.1051693404634581 - nodes in this community are weakly interconnected._
- **Should `Transcript Content Blocks` be split into smaller, more focused modules?**
  _Cohesion score 0.1067193675889328 - nodes in this community are weakly interconnected._
- **Should `Transcript Record Parsing` be split into smaller, more focused modules?**
  _Cohesion score 0.11092436974789915 - nodes in this community are weakly interconnected._