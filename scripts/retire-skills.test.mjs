...
check('a dry run reports every copy and removes none', () => {
  ...
  const reported = retireSkills(root, RETIRED, true)
  assert.equal(reported.length, 4, `expected 4 paths, got ${reported.length}`)
...
check('the real run removes the payload and the symlink into it', () => {
  const root = makeInstall()
  retireSkills(root, RETIRED, false)
  for (const name of RETIRED) {
...
check('a second run is a no-op', () => {
  const root = makeInstall()
  retireSkills(root, RETIRED, false)
  assert.deepEqual(retireSkills(root, RETIRED, false), [], 'second run reported removals')
})
...
check('a symlink whose payload is already gone is still removed', () => {
  ...
  const removed = retireSkills(root, ['visual-verify'], false)
...
check('nothing retired and nothing installed says nothing', () => {
  assert.equal(describeRetired([], 0, false), '')
  assert.match(describeRetired([], 3, false), /none installed/)
  assert.match(describeRetired(['.agents/skills/x'], 1, true), /dry run/)
})
