...
// `worktree-gc.harness.mjs`.
import assert from 'node:assert/strict'
import { check, report } from './check-harness.mjs'
import { scenario } from './worktree-gc.harness.mjs'
...
check('a design branch whose epic has closed is reaped', () => {
  ...
  const out = s.run()
  assert.match(out, /reaped branch design\/a-screen/)
...
check('a design branch whose epic is still open survives', () => {
  ...
  s.branch('design/a-screen')
  const out = s.run()
  assert.match(out, /keep branch design\/a-screen — epic #700 is OPEN/)
  assert.ok(s.remoteBranches().includes('design/a-screen'), out)
  s.cleanup()
})
...
check('a design that names no epic keeps its branch', () => {
  ...
  s.branch('design/a-screen')
  const out = s.run()
  assert.match(out, /keep branch design\/a-screen — .* names no epic/)
  assert.ok(s.remoteBranches().includes('design/a-screen'), out)
  s.cleanup()
})
...
check('an unreadable issue keeps the branch', () => {
  ...
  s.branch('design/a-screen')
  const out = s.run()
  assert.match(out, /keep branch design\/a-screen — epic #700 is unreadable/)
  assert.ok(s.remoteBranches().includes('design/a-screen'), out)
  s.cleanup()
})
...
check('--dry-run reports the reap and deletes nothing', () => {
  ...
  assert.match(out, /reap branch design\/a-screen — epic #700 closed \(dry run\)/)
  assert.ok(s.remoteBranches().includes('design/a-screen'), out)
  s.cleanup()
})
