...
provider's HTTP API, not the `gh` CLI** (tokens in the OS keychain). Every port reconciles through
that API. GitHub's webhook forwarder can invalidate Code host and Ticket reads without a public
endpoint, so its socket is a fast path. Conditional API reads remain the correctness floor. Linear
stays polled until Argo has a secure HTTPS webhook relay (ADR-0018, ADR-0047). A push event never
writes provider truth directly. `gh` remains how *agents* operate the repo — a different layer.
