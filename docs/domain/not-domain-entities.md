...
**Fold** — a Session list grouping, and never an entity. When headless Sessions are present, one row stands for those
(L2 · Entry) that share a working directory. It has no id beyond that directory, the Hub has never
heard of it, and nothing about a Session is stored on it: it is something the Session list does to
...
Session that FAILED reachable, which is the whole reason the Session list folds these rows instead of
dropping them.
