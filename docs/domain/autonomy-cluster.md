...
- **Mode** — the **Session's** *standing* autonomy stance (root-Agent fact, not per-Subagent): a
  ...
  DIRECT with a live Argo channel, tier-gated without one. Each rung is a **boundary**, not a prompt
...
- **Permission** — a *per-action* prompt the **agent** raises ("may I run this tool?"; options
  `allow_once · allow_always · reject_once · reject_always`). DIRECT with a live channel; drives the
  `permission` session-status.
- **Standing allow** — one **tool** a Session has stopped raising Permissions for, made by
  answering a Permission with *always*. DIRECT with a live channel, keyed by the CLI's tool name
...
- **Permission expiry** — a Permission that ended because **Argo's own patience ran out**, not
  because anyone answered it (#573). DIRECT with a live channel: the gate's clock is deliberately
  ...
  **Survives no restart**, for the reason a Standing allow does not — the gate that would have
  observed it dies with the live channel, so an Argo restart records nothing about that Permission.
