...
- **Token contract** — `apps/desktop/src/renderer/styles/globals.css` and its local CSS imports
  form one contract. Shared Session values live in `styles/session-tokens.css`; `styles/typography.css`
  maps the approved `type-*` roles to those values. Raw values belong in the contract.
