# Module boundaries — the nine edges and why each is a gate

Companion to `AGENTS.md` → *Module boundaries*. That section carries the layer order and the fact
that `scripts/swift-boundaries.sh` enforces it; this one carries what each edge's subject actually
is, and the argument that made it a build failure rather than a review note. Read it when you are
adding a target, moving a type between modules, widening a `public` name, or a boundary check has
just failed and the message alone does not say why the rule exists.
