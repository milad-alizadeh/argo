...
| `cockpit-roster-row.inventory.md` | Roster row build inventory | What #1346's build actually extracted from that design — `DeliveryAddresses`, its two custom pull-request marks, and what stayed inline — plus the app-level wiring #389 still owes before a production row draws a real pull request |
