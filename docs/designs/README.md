...
| `cockpit-backlog-question.md` + `.html` + `backlog-question/` | **Approved asking-surface design** (#1293) | The second path for the question the search field cannot answer. One field, and the ask is FOUND rather than switched to: Search keeps the return key, the ask takes a modifier, and the row of controls grows nothing. The answer draws as prose on a sheet over the ticket pane, names what it read and which Account paid, and every number in it is checked against the set the room holds — one that is not there draws dead. Ten state renders in `backlog-question/` are the spec. **Covers the asking surface only**: the room around it is drawn to what ships, because `cockpit-work-room.html` is stale on the toolbar and the query (#1304) |
...
| `backlog-question-prototype.html` + `.md` | Where does a natural-language question about the backlog live, and what does it give back? (#1293) — settled by `cockpit-backlog-question.md`; the five rejected shapes stay switchable on the branch (`?variant=A…F`) |
