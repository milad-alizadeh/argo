...
| `cockpit-roster-row.inventory.md` | Roster row build inventory | What each ticket's build actually extracted from that design, appended per ticket — #1344 (`SubagentDots` and the `SubagentReading` value it draws, off the same reading the Agents rail counts from, plus the two tokens it promoted) and #1346 (`DeliveryAddresses`, its two custom pull-request marks, and the app-level wiring #389 still owes before a production row draws a real pull request) |
