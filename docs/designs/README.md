...
| `cockpit-roster-row.inventory.md` | Roster row build inventory | What #1344's build actually extracted from that design — `SubagentDots` and the `SubagentReading` value it draws, off the same reading the Agents rail counts from — plus the two tokens it promoted and what stayed inline |
