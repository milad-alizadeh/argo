...
| `cockpit-feed-working.md` + `working/` | **Approved in-flight design** | What the feed draws while a Session is `running`: the ion across a pending call's type, the thread across the measure while thinking, and the rule that only ever one of them shows. Four renders in `working/` are the spec; the doc carries the measurements and the seven contract changes it needs |
