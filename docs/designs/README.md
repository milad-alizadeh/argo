...
| `cockpit-work-room.md` + `work-room/` | **Approved Work-room design** (#609, under #607) | The room #376's shell had a tab for and nothing behind: the backlog rail as a disclosure tree with priority over it, the Next-up hero as a card inside the rail, the ticket detail against a 240pt facts sidebar, both room-level vacancies, and the Route re-skinned onto graphite/Ion with #334's geometry untouched. Ten state renders in `work-room/` are the spec; the doc carries the measurements, the frozen names and the one proposed role |
...
| `work-room-prototype.html` + `.md` | What does the Work room look like in the Liquid Glass shell? (#609) — settled by `cockpit-work-room.md`; the two rejected rooms stay switchable on the branch |
