...
| `cockpit-work-room.inventory.md` | Work-room build inventory | What each ticket's build actually extracted from that design, one row per component — #812 (the views sidebar, the flat backlog and the ticket) and #815 (the fact strip, the Delivery chips and the one link list), plus what stayed inline, the `sessionTitle` role it promoted, the three places the design's own names and counts do not match the contract, and the six honesty calls #815 had to make where the explorable knew more than Swift can |
