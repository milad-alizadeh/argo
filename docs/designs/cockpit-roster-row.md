...
| Subagent dot | **4** | `ArgoIconSize.subagentDot`, promoted by the build (#1344). Smaller than the state dot, which is what "half the state dot" was reaching for — 4 is two thirds of 6, and the number is what the table, the `.html` and the ten renders all state, so it is the number that ships. The prose was the loose half, not the figure |
