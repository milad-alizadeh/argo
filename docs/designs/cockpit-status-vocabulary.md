...
| Running | `running` | green, live glow | the normal state with 3–4 sessions; liveness stays legible under attention. **DERIVED**, except between a Turn Argo itself typed at a managed Session and the record answering it, which is DIRECT (`CONTEXT.md` L2, #1048). Nothing here renders the difference — the word and the glow are the same either way — so this row is one entry, not two |
