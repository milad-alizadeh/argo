...
- **`CommandMenuProjection`** — the container/View split `apps/macOS/AGENTS.md` requires. Every rule
  the design states about ORDER is here rather than in a `body`.
...
- **`FeedSkillLoad`** — the container/View split `apps/macOS/AGENTS.md` requires. It also carries
  the one fact the engine's `SkillLoad` cannot answer alone: whether the file read lies outside the
...
the state here, the way `apps/macOS/AGENTS.md` says it is for anything laid out in columns.
