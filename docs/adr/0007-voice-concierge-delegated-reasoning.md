...
4. **We inherit an authoring default — but a weaker one than first stated.** In the Chat-Supervisor *reference pattern* the reasoner authors the spoken line and the audio model reads it verbatim, which looked like it pre-empted [#212](https://github.com/milad-alizadeh/argo/issues/212). Source-reading OpenAI's shipped client ([#212](https://github.com/milad-alizadeh/argo/issues/212) comment) corrects this: the current path carries an explicit `speakable | commentary` channel on the return leg and a **mode selector** whose *default* (`Thinking`) sends no channel and lets the audio model decide what to say. Verbatim-read is one selectable mode, not the default. So the inherited default points at **the concierge reshaping — Argo's status quo** — and #212 stays genuinely open rather than pre-empted.
