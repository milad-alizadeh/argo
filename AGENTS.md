...
than review notes. Nine edges; the sharpest is **exactly one file in `ArgoUI` may read live Hub
state** — the Hub → cockpit projection. Everything else takes a value.

What each edge's subject is and the argument behind it — ADR-0022's layering, ADR-0027's
restatement rule, the init-cap and token-contract ratchets, ADR-0030 Rule 1, and the dormant
JS/TS gates: `docs/agents/module-boundaries.md`. Read it before adding a target, moving a type
between modules, or widening a `public` name.
