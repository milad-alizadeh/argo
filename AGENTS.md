...
- **Issue tracker** — Issues and PRDs live in GitHub Issues on `milad-alizadeh/argo`, via the
  `gh` CLI. Every issue you create carries a triage label in the `gh issue create` call —
  there is no unlabelled issue, and a bug report is no exception. A screenshot goes in the
  issue body, and in the PR body when a screen changes. See `docs/agents/issue-tracker.md`.
