...
**Session**:
One stock agentic CLI run (a `claude` or `codex` process) observed by the cockpit — discovered from the CLI's own transcript on disk. The top-level unit on the Roster. **Managed** Sessions are additionally spawned and wrapped in a PTY by the cockpit (enabling the companion plugin and steering); **external** Sessions (started in a plain terminal) are observed read-only from their transcript. All Sessions are observed; only managed ones carry the CONVENTION-tier derived layer. A Session spans a **resume-chain** of one-or-more CLI session files (linked by `leafUuid`), stitched at read time so Outcomes carry across a resume.
_Avoid_: process, tab, terminal

**Roster**:
The cockpit's left-hand list of observed Sessions — the top-level navigation surface from which a Session is selected. Each row is a Session; its word/tone/icon derive from the same source as that Session's Delivery lifecycle, so the two can never disagree.
_Avoid_: rail, sidebar, nav
...
_Avoid_: actor (concurrency / message-passing sense), node
...
**Panels**:
The four toggleable regions of an Actor's screen: **Activity** (time-keyed prose), **Delivery** (sha-keyed product), **Preview** (the running UI), **Console** (raw I/O). One button group selects them; each panel repeats its own icon in its header. Supersedes the Story/Work naming of ADR-0009.
_Avoid_: Story, Work, Detail, tab, pane
...
**Gate**:
A Delivery-lifecycle transition that publishes or lands work (Create PR, Merge). Human-clicked by default; delegable per session as a standing order (rendered `⚙ auto`, revocable) — the only sanctioned form of the cockpit acting without a per-action gesture.
