...
**Actor**:
The recursive node of the observability tree. A Session and an Agent are both Actors; an Actor may own Runs, and an Agent leaf may itself own Runs (genuine recursion).
...
**Honesty tier**:
The provenance grade of any field the cockpit renders. **DIRECT** (clean from transcript/git, zero inference) · **DERIVED** (computed/estimated, fragile) · **CONVENTION** (requires an Argo skill/plugin to own the prompt+output). A field renders per its tier; nothing is FABRICATED.
