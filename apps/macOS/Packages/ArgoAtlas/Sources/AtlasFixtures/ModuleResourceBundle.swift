...
/// running process finds that out on first touch. `AtlasMapFixture` and `AtlasNotesFixture` already
/// throw a value error when their file is missing from the bundle; resolving the bundle itself this
/// way is what lets that guard actually run instead of trapping one line earlier.
enum ModuleResourceBundle {
    ...
    static let resolved: Bundle? = resolve(bundleName: "ArgoAtlas_AtlasFixtures")
