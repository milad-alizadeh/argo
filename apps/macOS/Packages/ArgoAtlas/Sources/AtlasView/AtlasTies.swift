...
/// it — the strongest across the map, and the open file's own — and a caller that chose one of
/// them here would have to choose the other one too, in a column that does not know what is open.
///
...
public struct AtlasTies: Equatable, Sendable {
    ...
    /// Whether Strongest ties is on. It governs the whole-map reading only: an open file draws its
    /// own however this is set, because the reader pointed at a file and asked a question the
...
///
/// The open file's own answer a question the reader just asked, so they are the heavier; the
/// strongest across the map are a texture over the whole picture, and at the open weight a hundred
/// and sixty of them would be the only thing on the frame.
///
/// Public because the colour contract's own sheet draws both weights, and a sheet asking whether a
/// cord is visible over the deepest plate has to ask it at the width the map actually strokes.
public enum AtlasCordWeight: Equatable, Sendable, CaseIterable {
    /// One of the strongest across the whole map, drawn because the switch is on.
    case acrossTheMap
    /// One of the open file's own.
    case ofTheOpenFile
    ...
    public var width: CGFloat {
        ...
        case .ofTheOpenFile: ArgoStroke.indicator
        }
    }
}
