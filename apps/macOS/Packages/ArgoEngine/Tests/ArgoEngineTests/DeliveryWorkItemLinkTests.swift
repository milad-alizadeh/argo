...
/// Which Work Item a branch serves, by the precedence ADR-0014 fixed: native reference →
/// id-in-branch → unlinked.
@Suite("Delivery to Work Item join")
struct DeliveryWorkItemLinkTests {
    ...
    func `a closing keyword is what makes a body name a ticket`(_ example: KeywordCase) {
        ...
        #expect(link == example.number.map(DeliveryWorkItemLink.native) ?? .unlinked)
    }
    ...
    @Test
    func `an unlinked Delivery names no Work Item at all`() {
