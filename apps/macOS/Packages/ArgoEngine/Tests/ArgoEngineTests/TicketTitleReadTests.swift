...
@Suite("Ticket title read")
struct TicketTitleReadTests {
    ...
    private static func read(
        ...
        -> (TicketTitleReading?, StubProviderAPI) {
        let api = StubProviderAPI(body: body, failure: failure)
        let read = await GitHubTicketTitles(transport: api)
    ...
    @Test
    func `a pull request number is not a Ticket`() async {
        // GitHub serves PRs from the same `/issues/<N>` path, and a PR title in the ticket rung
        // would name a Delivery where the row claims a Ticket (`CONTEXT.md` L1, L4).
