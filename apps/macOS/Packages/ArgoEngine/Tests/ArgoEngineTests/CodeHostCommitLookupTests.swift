...
struct CodeHostCommitLookupTests {
    ...
    func `a commit the repository does not hold reads as no Delivery`() async throws {
        ...
        // as a thrown status. Read as a refusal it would break the whole per-branch fan-out.
    ...
    private static let reopened = PullRequestJSON(number: 1700, updatedAt: "2026-09-01T00:00:00Z")
    private static let older = PullRequestJSON(number: 1500, updatedAt: "2026-07-01T00:00:00Z")
}
