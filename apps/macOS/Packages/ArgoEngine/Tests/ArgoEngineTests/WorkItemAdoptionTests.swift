...
/// (#257, #260).
@Suite("Work Item write adoption")
struct WorkItemAdoptionTests {
    private static let project = "argo"

    private struct Writing {
        let writer: WorkItemWriter
    ...
    private static func writing(
        ...
        return Writing(
            writer: WorkItemWriter(
                port: GitHubWorkItems(transport: api), items: items, health: health,
            ),
            items: items,
            health: health,
            api: api,
    ...
    func `a written ticket replaces the one the room was holding`() async throws {
        let listed = WorkItem(number: 12, title: "Old title", status: "open", closure: .open)
    ...
    func `a filed ticket joins the room it was filed into`() async throws {
        ...
        _ = try await writing.writer.create(WorkItemDraft(title: "New"), on: Self.target)
    ...
    func `the provider's own reason reaches the caller`() async throws {
        ...
        let refusal = await #expect(throws: WorkItemWriteError.self) {
            try await writing.writer.apply(
                .updateFields(WorkItemFields(title: "")), to: 12, on: Self.target,
            )
        }
    ...
    func `a write that never reached the provider is recorded as a fault`() async {
        ...
        let health = ConnectionHealthLedger()
        let writer = WorkItemWriter(
            port: GitHubWorkItems(transport: api), items: WorkItemLedger(), health: health,
        )
