...
@Suite("Ticket writes")
struct TicketWriteTests {
    private static func write(
        _ intent: TicketIntent, replies: [String: String] = [:],
    ) async throws
        -> RecordedWrite {
        let api = RecordedGitHub(replies: Self.issues.merging(replies) { _, given in given })
        _ = try await GitHubTickets(transport: api).apply(intent, to: 12, through: .stub())
    ...
    struct TransitionCase: Sendable {
        let state: TicketCanonicalState
    ...
    func `a state GitHub cannot express costs no request at all`() async throws {
        ...
        await #expect(throws: TicketWriteError.inexpressible(.inProgress)) {
            try await GitHubTickets(transport: api)
    ...
    func `a priority GitHub has no field for is written as the label it is read from`(
        ...
        _ = try await GitHubTickets(transport: api).apply(
