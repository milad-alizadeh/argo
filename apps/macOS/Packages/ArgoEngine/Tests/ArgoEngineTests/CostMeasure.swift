...
/// intrinsic cost from above where a mean carries whatever else the machine was doing.
func leastCPUSeconds(trials: Int, of work: () -> Void) -> Double {
    var least = Double.greatestFiniteMagnitude
    for _ in 0 ..< trials {
        let started = threadCPUSeconds()
        work()
        least = min(least, threadCPUSeconds() - started)
    }
    return least
}
