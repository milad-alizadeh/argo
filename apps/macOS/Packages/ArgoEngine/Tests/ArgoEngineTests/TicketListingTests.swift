...
struct TicketListingTests {
    private static func list(
        ...
        -> ([Ticket], RecordedGitHub) {
        let api = RecordedGitHub(replies: replies)
        let items = try await GitHubTickets(transport: api).list(in: "acme/api", grant: .listing)
        return (items, api)
    }
    ...
    @Test
    func `a pull request served from the issues path is not a Ticket`() async throws {
    ...
    func `each blocker carries the closure it was verified at`() async throws {
        ...
        #expect(items.first?.blockedBy == [
            TicketBlocker(number: 1, closure: .ruledOut),
    ...
    @Test
    func `only open Tickets are listed`() async throws {
