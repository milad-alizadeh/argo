...
///
/// Serialized for the reason `PermissionChannelTests` is — every test here waits on the main actor
/// for a `DispatchSource` on the main queue.
@Suite("Ask channel", .serialized)
@MainActor
struct AskChannelTests {
    ...
    @Test
    func `the plugin puts the ask through the same gate a permission goes through`() async throws {
    ...
    func `an AskUserQuestion raises a live ask the Session reads DIRECT`() async throws {
        try await PermissionGate.withGate { fixture, _, client in
            ...
            // It is a question, never a Permission: the two are answered by different acts and the
    ...
    /// user's behalf.
    @Test
    func `a question is put even on Auto, where a gated call would be waved through`() async throws {
    ...
    /// The hook went while Argo was still willing to wait, so its turn was cancelled: the question
    ...
    /// A call NAMED `AskUserQuestion` whose input carried no readable question is not one anybody
    /// can answer, so it falls through to the gate's ordinary reading rather than raising a blank.
    @Test
    func `an AskUserQuestion with no question in it is a Permission like any other`() async throws {
        try await PermissionGate.withGate { fixture, _, client in
            client.sendLine(#"{"tool_name":"AskUserQuestion","tool_input":{"questions":[]}}"#)
            await settle { fixture.hub.sessions.first?.permission != nil }

            #expect(fixture.hub.sessions.first?.ask == nil)
            #expect(fixture.hub.sessions.first?.permission?.toolName == ToolCall.askUserQuestion)
        }
    }
