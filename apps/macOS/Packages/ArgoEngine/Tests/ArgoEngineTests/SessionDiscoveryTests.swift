...
struct SessionDiscoveryTests {
    ...
    @Test
    func `the working set is this Project's recently written transcripts`() throws {
        ...
        let discovered = SessionDiscovery(store: fixture.store).workingSet(for: projectURL)
    ...
    @Test
    func `a sibling sharing a name prefix is not inside the Project`() throws {
        let fixture = try RecordDirectoryFixture()
        defer { fixture.remove() }
        let projectURL = URL(fileURLWithPath: fixture.path("checkout"))
        try fixture.write(FixtureTranscript(name: "sibling", cwd: fixture.path("checkout-two")))

        #expect(SessionDiscovery(store: fixture.store).workingSet(for: projectURL).isEmpty)
    }

    /// A session started in a subdirectory — or in a worktree kept under the repo — is working on
    /// this Project.
    @Test
    func `a transcript below the Project root is inside it`() throws {
        let fixture = try RecordDirectoryFixture()
        defer { fixture.remove() }
        let projectURL = URL(fileURLWithPath: fixture.path("checkout"))
    ...
    @Test
    func `a transcript reporting no working directory is attributed to no Project`() throws {
        let fixture = try RecordDirectoryFixture()
        defer { fixture.remove() }
        let projectURL = URL(fileURLWithPath: fixture.path("checkout"))
    ...
    @Test
    func `a sidechain below a project directory is not a Session`() throws {
        let fixture = try RecordDirectoryFixture()
        defer { fixture.remove() }
        let projectURL = URL(fileURLWithPath: fixture.path("checkout"))
        try fixture.write(FixtureTranscript(
    ...
    /// failure to report.
    @Test
    func `a missing record directory sweeps to nothing`() {
        ...
        #expect(discovery.workingSet(for: URL(fileURLWithPath: "/tmp/checkout")).isEmpty)
    }

    @Test
    func `the working set is newest first`() throws {
        let fixture = try RecordDirectoryFixture()
        defer { fixture.remove() }
        let projectURL = URL(fileURLWithPath: fixture.path("checkout"))
