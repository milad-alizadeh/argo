...
struct SkillCatalogJoinTests {
    ...
    @Test func `a Project's own skills are in the catalog read for it`() throws {
        let fixture = try BuiltinReaderFixture()
        let project = try Self.project(holding: "ship")
        defer { try? FileManager.default.removeItem(at: project) }

        let catalog = fixture.reader().catalog(forProjectAt: project)
    ...
    @Test func `a Project with no skills folder contributes none`() throws {
        let fixture = try BuiltinReaderFixture()
        let nowhere = URL.temporaryDirectory.appending(path: "argo-none-\(UUID().uuidString)")

        let catalog = fixture.reader().catalog(forProjectAt: nowhere)
