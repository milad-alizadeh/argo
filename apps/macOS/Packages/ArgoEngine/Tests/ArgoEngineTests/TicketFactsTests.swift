...
@Suite("Ticket facts")
struct TicketFactsTests {
    private static func list(_ issues: [IssueJSON]) async throws -> [Ticket] {
        let api = RecordedGitHub(replies: [RecordedGitHub.openIssues: IssueJSON.list(issues)])
        return try await GitHubTickets(transport: api).list(in: "acme/api", grant: .listing)
    }

    private static func first(_ issue: IssueJSON) async throws -> Ticket {
