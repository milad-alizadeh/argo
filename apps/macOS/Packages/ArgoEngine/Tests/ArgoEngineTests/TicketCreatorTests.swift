...
@Suite("Ticket creator")
struct TicketCreatorTests {
    private static let draft = TicketDraft(title: "Wire the verbs", body: "Prose.")

    private static func creator(
        _ fixture: BindingFixture, api: StubProviderAPI, items: TicketLedger,
    )
        -> TicketCreator {
        TicketCreator(
            ...
            writes: ProviderTicketWrites(transport: api),
        )
    }
    ...
    @Test func `a Project with no Ticket Binding has nowhere to file`() async throws {
        let fixture = try BindingFixture()
        defer { fixture.remove() }
        let projectID = try await fixture.project("argo")
        let api = StubProviderAPI()

        let refusal = await Self.creator(fixture, api: api, items: TicketLedger())
            .create(Self.draft, forProject: projectID)
