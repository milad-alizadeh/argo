...
/// indistinguishable from one that is genuinely malformed. The tail reads forwards from a cursor
/// and the excerpt reads the two ends (`TranscriptExcerpt`); both need this and neither needs a
/// second copy of it.
///
/// The offset is taken from the RAW parts, before any of them is turned into a `String`: a line
/// that is not UTF-8 is still bytes the file counted, and dropping it from the tally would
/// mis-address every picture after it (`MediaLocation`).
