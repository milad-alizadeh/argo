...
extension TranscriptReader {
    ...
    private func callEvents(_ use: ToolUseBlock, in message: MessageRecord) -> [TranscriptEvent] {
        ...
        let call = ToolCall(
            ...
            // Gated the same way, on the qualified name the companion channel's own tool carries
            // (#1335) — the socket already read the SAME call for the roster's claim; this is
            // the transcript's own reading of it, for the feed.
