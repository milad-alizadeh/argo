...
final class TranscriptWatch {
    ...
    /// What must run once a SWEEP has moved the tails. A batch is not the only thing that changes
    ...
    ///
    /// Separate from `onApplied` because the two answer different halves. A batch changes what a
    /// row SAYS, and a sweep changes which rows there are.
    @ObservationIgnored var onSwept: @MainActor () -> Void = {}
