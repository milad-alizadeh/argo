...
final class ClaimLedger {
    ...
    ///
    /// It retires a question the agent asked over the channel in the same write (#1203), because
    /// those two are one act: the composer is where a reported question is answered, so the Turn
    /// going down the PTY IS the answer. Which of the report's facts that touches is
