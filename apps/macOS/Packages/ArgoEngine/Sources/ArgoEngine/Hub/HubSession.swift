...
public struct HubSession: Equatable, Identifiable, Sendable {
    ...
    /// The ids the links folded into this row were published under, oldest first, and empty for a
    /// chain of one. A continuation read before its origin stands as a Session of its own
    /// (`HubJoinPublishable`), and the sweep that finds the origin absorbs it: one row, under an
    /// id the reader was never pointed at. Kept so a surface holding the absorbed id can follow it
    /// to the row that took it, rather than read the Session as gone (#1481).
    public private(set) var absorbedIDs: [String] = []
...
extension HubSession {
    mutating func mergeContinuation(_ continuation: HubSession) {
        // The absorbed link's own id, and every id it had already absorbed: a chain of three is
