...
/// Which Work Item a Session serves, read off its git context (`CONTEXT.md` L1 · Work Item).
///
...
/// confirms the Work Item exists — a number that resolves to nothing degrades to no link.
///
...
public enum WorkItemLink {
    /// The worktree folder's stem, as `docs/agents/worktrees.md` fixes it.
    private static let worktreePrefix = "ticket-"

    /// The Work Item number this git context names, and `nil` where neither half names one.
    ///
    ...
    /// number: providers number Work Items from one, so `#0` is a misread rather than a link.
