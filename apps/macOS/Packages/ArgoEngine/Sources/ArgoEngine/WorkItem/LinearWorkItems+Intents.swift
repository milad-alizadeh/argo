...
extension LinearWorkItems {
    ...
    func operation(
        for intent: WorkItemIntent, on id: String, through binding: ResolvedBinding,
    ) async throws
        -> LinearOperation {
        switch intent {
    ...
    private func moving(
        to state: WorkItemCanonicalState, on id: String, through binding: ResolvedBinding,
    ) async throws
        -> LinearOperation {
    ...
    private static func edited(_ fields: WorkItemFields) -> [String: LinearValue] {
