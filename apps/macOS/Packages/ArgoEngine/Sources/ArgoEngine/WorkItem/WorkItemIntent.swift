...
/// One change the cockpit asks a Work Item provider for, said canonically rather than in the
/// provider's own shape (#167, `CONTEXT.md` → Ports).
///
/// Creating a ticket is not here because it has no ticket to be applied to — it is
/// `WorkItemWriting.create`, and the two together are the eight the port declares over.
public enum WorkItemIntent: Equatable, Sendable {
    case updateFields(WorkItemFields)
    case transitionTo(WorkItemCanonicalState)
    case addBlockedBy(Int)
    case removeBlockedBy(Int)
    case setParent(Int)
    /// Detach from a NAMED parent: a Work Item carries its children and never its parent, so the
