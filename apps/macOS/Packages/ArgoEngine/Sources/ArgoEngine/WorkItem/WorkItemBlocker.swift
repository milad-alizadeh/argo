...
/// One edge of a Work Item's `blockedBy` DAG, carrying the blocker's own closure rather than a
/// reference to it (`CONTEXT.md` L1 · Work Item).
///
/// The closure travels ON the edge because the domain verifies blockers per-blocker: the
/// provider's summary count is stale, and it counts open blockers only, so a map read off it
/// cannot tell a cleared edge from one whose blocker was cancelled.
...
public enum WorkItemBlockage: Equatable, Sendable {
    ...
    /// Stranded outranks blocked, because a blocked ticket clears itself in time and a stranded
    /// one never will.
