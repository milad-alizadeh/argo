...
/// GitHub Issues as the write half of the Work Item port (#257, #167).
///
...
extension GitHubWorkItems: WorkItemWriting {
    public var surface: WorkItemSurface {
        WorkItemSurface(writes: Set(WorkItemWrite.allCases), states: [.todo, .done, .closed])
    }
    ...
    /// The issue as a Work Item, edges and all — the same shape a poll produces, so an adopted
    ...
    private func adopted(
        ...
        -> WorkItem {
        do {
            return try await workItem(issue, in: binding.binding.scope, grant: binding.grant)
        } catch let error as ProviderFetchError {
            throw WorkItemWriteError.unreachable(error)
        }
    }
}
