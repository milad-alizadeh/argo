...
/// The issue as a Ticket — the one place Linear's shape becomes Argo's.
extension LinearIssue {
    ...
    func ticket() -> Ticket {
        Ticket(
            ...
            labels: labels.nodes.map {
                TicketLabel(name: $0.name, colour: LinearAPI.bareHex($0.color))
            },
            priority: priority,
    ...
    private var blockers: [TicketBlocker] {
        inverseRelations.nodes
            .filter { $0.type == LinearTickets.blocks }
            .map {
                TicketBlocker(number: $0.issue.number, closure: $0.issue.state.category.closure)
            }
    }
}
