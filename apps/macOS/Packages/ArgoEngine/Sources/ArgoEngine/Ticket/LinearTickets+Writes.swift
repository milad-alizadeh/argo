...
/// Linear as the write half of the Ticket port (#371, #257, #167).
///
...
extension LinearTickets: TicketWriting {
    public var surface: TicketSurface {
        TicketSurface(
            writes: Set(TicketWrite.allCases),
    ...
    public func create(
        ...
        guard let number = filed.issue?.number else {
            throw TicketWriteError.refused("Linear filed the ticket without saying its number.")
        }
    ...
    private func sent(
        ...
        guard reply.result.success else {
            throw TicketWriteError.refused("Linear did not apply the change.")
        }
        return reply.result
    }
