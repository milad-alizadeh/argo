...
///
/// It writes the annotation file and types NOWHERE. It used to also type the Ticket's words at the
/// Session's prompt (#1494), and that was the second of two mirrors: it typed `reading.title` raw
/// while `SessionNameMirror` types the name the roster SPELLS, so whichever won the race left the
/// phone on a spelling the desk never draws, and neither could correct the other (#1623). One
/// typist now, and a Ticket's words reach the prompt the same way every other name does — as the
/// name a row draws.
