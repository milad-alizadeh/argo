...
extension LinearTickets {
    ...
    func identifier(of number: Int, through binding: ResolvedBinding) async throws -> String {
        guard let issue = try await found(number, through: binding) else {
            throw TicketWriteError.refused("Linear holds no issue \(number) in this team.")
        }
        return issue.id
    }
    ...
    func stateID(
        for state: TicketCanonicalState, through binding: ResolvedBinding,
    ) async throws
        -> String {
    ...
    func labelID(named name: String, through binding: ResolvedBinding) async throws -> String {
        ...
        guard let label = payload.issueLabels.nodes.first else {
            throw TicketWriteError.refused("This workspace has no label called \"\(name)\".")
        }
        return label.id
    }
    ...
    func relationID(
        ...
        let edge = payload.issue?.inverseRelations.nodes.first {
            $0.type == LinearTickets.blocks && $0.issue.number == number
        }
        guard let edge else {
            throw TicketWriteError.refused("Issue \(number) does not block this one.")
        }
        return edge.id
    }
    ...
    private func states(through binding: ResolvedBinding) async throws -> [LinearWorkflowState] {
        ...
        guard let team = payload.team else {
            throw TicketWriteError.refused("This account cannot see the team.")
        }
