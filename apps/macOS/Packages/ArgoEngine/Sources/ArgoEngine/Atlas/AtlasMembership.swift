...
struct AtlasMembership {
    ...
    /// The community each file kept, `AtlasAgreement.unplaced` where it kept none.
    let belonging: [Int]
    ...
    init(of belonging: [Int], over graph: AtlasGraph, margin: Double = AtlasMembership.margin) {
        var sizes: [Int: Int] = [:]
        for community in belonging {
            sizes[community, default: 0] += 1
        }
        var kept = [Int](repeating: AtlasAgreement.unplaced, count: belonging.count)
        ...
        for node in belonging.indices where (sizes[belonging[node]] ?? 0) >= Self.smallest {
            ...
            guard held >= margin else { continue }
            kept[node] = belonging[node]
            confidence[node] = held
        }
        // The floor again, on what is LEFT. It was applied to the communities the partition
        ...
        var held: [Int: Int] = [:]
        for community in kept where community != AtlasAgreement.unplaced {
            held[community, default: 0] += 1
        }
        for node in kept.indices where (held[kept[node]] ?? 0) < Self.smallest {
            kept[node] = AtlasAgreement.unplaced
            confidence[node] = 0
        }
        self.belonging = kept
