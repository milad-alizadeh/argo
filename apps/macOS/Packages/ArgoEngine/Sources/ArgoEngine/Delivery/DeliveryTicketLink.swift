...
public enum DeliveryTicketLink: Equatable, Sendable {
    ...
    public static func derived(
        ...
        if let number = TicketLink.number(branch: branch, workspaceLocation: nil) {
            return .idInBranch(number)
        }
        return asserted.map(DeliveryTicketLink.asserted) ?? .unlinked
    }
}
