...
public actor DeliveryDerivation {
    ...
    private struct Assembled {
        ...
        /// The refusal that cut the fan-out short, as it was thrown. Unclassified, because a
        /// health word here has no room for "there is no word for this": that would be `nil`,
        /// which this field already spends on "nothing refused the read" — a success (#1698).
        var refusal: Error?
    ...
    /// silent (#1643). It stays silent for a dial Argo's own code refused: the ledger has no word
    /// for such an error, and every word it does have would be a fault the reader cannot act on
    /// (#1698).
