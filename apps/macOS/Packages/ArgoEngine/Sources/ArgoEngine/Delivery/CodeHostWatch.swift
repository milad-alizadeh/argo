...
///
/// A fast path and never the only one: every reason this port has to refuse is a throw the caller
/// degrades to the poll on, and the poll stays at its own interval regardless (#1579).
...
public enum DeliveryWatchRefusal: Error, Equatable {
    /// The host answered the create and offered no socket to dial. Told apart from a host nothing
    /// could reach, because this one answered: a hook without a `ws_url` is one GitHub made the
    /// ordinary way, which is what a withdrawn forwarder would leave behind.
    case noSocketOffered
}
