...
/// the connection.
///
/// `URLError.Code` is a struct over about ninety values rather than an enum, so this cannot be
/// exhaustive the way `HTTPTransportError.fetchFailure` is: the two sets below are named, and
/// every code outside them reached the wire and failed there.
extension URLError {
    ...
    private static let askedNothing: Set<URLError.Code> = [
