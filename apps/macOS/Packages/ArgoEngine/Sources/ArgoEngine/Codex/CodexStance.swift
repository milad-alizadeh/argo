...
///
/// Two values rather than one, because Codex decides a boundary with both: `approvalPolicy` says
...
struct CodexStance: Equatable {
    ...
    static func of(_ mode: SessionMode) -> CodexStance {
        switch mode {
        case .readOnly, .plan: CodexStance(approvalPolicy: "on-request", sandbox: .readOnly)
        case .code: CodexStance(approvalPolicy: "on-request", sandbox: .workspaceWrite)
        case .auto: CodexStance(approvalPolicy: "never", sandbox: .fullAccess)
        }
    }
}
