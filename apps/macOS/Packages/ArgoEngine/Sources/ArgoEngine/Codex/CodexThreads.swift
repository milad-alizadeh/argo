...
final class CodexThreads {
    ...
    /// reach it.
    ///
    /// The gate is closed first. Its clocks are `Task`s sleeping for a day against a server that
    /// has exited, and dropping the thread alone would leave them running with nothing to answer.
