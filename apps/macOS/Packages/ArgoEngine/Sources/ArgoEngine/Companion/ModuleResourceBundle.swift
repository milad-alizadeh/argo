...
/// running process finds that out on first touch, which takes the whole app down rather than the
/// one spawn that needed the resource. Mirroring the walk here, ending in `nil`, is what lets
/// `CompanionPlugin` answer "missing" instead of dying on the question.
enum ModuleResourceBundle {
    ...
    static let resolved: Bundle? = resolve(bundleName: "ArgoEngine_ArgoEngine")
