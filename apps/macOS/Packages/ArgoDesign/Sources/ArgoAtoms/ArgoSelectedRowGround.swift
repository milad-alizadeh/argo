...
public extension View {
    /// The ground under a selected sidebar row, in both rails — and the ONLY selection paint in the
    /// row. `.listStyle(.sidebar)`'s own fill, which on macOS 26 neither `.tint` nor the
    /// `AccentColor` asset moves by a value (D30, as amended by #875, #906, #922), is switched off
    /// at the table by the probe this carries (`SidebarSelectionFill`, #1137): the platform paints
    /// the pressed row on mouse-down, and the binding this ground follows moves on mouse-up.
...
private struct SelectedRowGround: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            ...
            .background { SidebarSelectionFillProbe() }
