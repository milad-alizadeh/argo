...
/// What a row says when the agent already said it. A Claude Code host requires a description on
/// every command it runs, so the sentence the feed wants is already in the record.
...
struct FeedNarrationTests {
    ...
    /// Verbatim is the whole tier. Narrations are imperative-present and the feed's verbs are past,
    /// so a described row reads `Ran · List open issues`; the clash is kept, never re-tensed.
    @Test
    ...
    /// A described row keeps the same shape as every other: a verb, then the name of the thing.
    @Test
    ...
    func `the panel behind a described row opens on the command and not the sentence`() throws {
        ...
        // Asked of the call rather than through `opened`: a call the record never answered has no
        // steps to carry the address and opens no panel at all.
        #expect(call.address == .typed("gh issue list --limit 20"))
    }
    ...
    /// The delegating call's other input is the whole brief, which a one-line row must not take.
    @Test
    func `a delegated row says the short description`() {
        ...
        // The description is the only thing the delegating call names, so it is both.
        #expect(calls.first?.subject.captioned == "Review the feed's call reading")
        #expect(calls.first?.kind.verb == "Delegated")
    }

    /// A description is not a scraped field: it is the host asking the agent what the call was FOR,
    /// so an unclassified tool says it rather than naming itself.
    @Test
    ...
    /// A subject that already identifies itself keeps doing so, even if a description arrives:
    /// `Read`/`Edit`/`Write` and `Skill` carry none, and an MCP tool's own input is arbitrary.
    @Test(arguments: [
    ...
    /// One row of that table: a call with an address of its own, and what it has to keep saying.
    ...
    /// The disclosure is derived from the evidence and from nothing else — a subject shape must not
    /// put a chevron on a row with nothing behind it.
    @Test
    ...
    /// One command, narrated or not — the fixture every claim above is a reading of.
