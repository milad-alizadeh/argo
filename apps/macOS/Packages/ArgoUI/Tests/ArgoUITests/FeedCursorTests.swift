...
struct FeedCursorTests {
    ...
    private static func reading(byKeyboard: Bool = true) async -> Reading {
        let handle = FeedTableHandle()
    ...
    func `a reading nobody has arrowed through carries no cursor`() async {
        let coordinator = await Self.reading().coordinator
    ...
    @Test
    func `the keyboard arriving under a pointer draws no cursor`() async {
        let coordinator = await Self.reading(byKeyboard: false).coordinator
