...
struct SessionTicketTitleTests {
    ...
    func `a Session on a ticket branch reads the ticket on the roster and the header alike`(
        ...
        #expect(row.title == "Anchor the feed")
        #expect(SessionHeaderProjection.header(from: linked).title == "Anchor the feed")
    }
    ...
    func `an explicit name outranks the ticket's title`() throws {
        ...
        #expect(row.rename?.derived == "Anchor the feed")
    }
    ...
    func `the roster the ticket specimens render carries the case the ticket is about`() {
        ...
        // The shared ticket's number is on line 3 now, never repeated in the meta slot (#1347).
        #expect(rows.contains { $0.ticketNumber == 741 && $0.toldApart == nil })
        #expect(rows.contains { $0.title == "Draw a markdown file as the document it is" })
    }
