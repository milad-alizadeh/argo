...
struct FeedAskProjectionTests {
    ...
    func `a Session blocked on a question offers it`() throws {
        let live = try #require(FeedAskProjection.live(for: FeedFixture.askingSession()))

        #expect(live.sessionID == "session")
        #expect(live.askID == "ask-1")
        #expect(live.ask == FeedFixture.askedLive.ask)
    }

    @Test
    func `a Session blocked on nothing offers nothing`() {
        #expect(FeedAskProjection.live(for: FeedFixture.askingSession(ask: nil)) == nil)
    ...
    func `an undriveable Session draws no affordance at all`() {
        #expect(FeedAskProjection.live(for: FeedFixture.askingSession(access: .external)) == nil)
        #expect(FeedAskProjection.live(for: FeedFixture.askingSession(access: .orphaned)) == nil)
    }
    ...
    func `a question is identified by what it asked, not by where it was drawn`() {
        let other = Ask.Question(text: "Something else?", options: [])
        let asked = FeedAsk(
            ask: Ask(questions: [FeedFixture.askedQuestion]),
            isAnswered: false,
            answer: nil,
        )
