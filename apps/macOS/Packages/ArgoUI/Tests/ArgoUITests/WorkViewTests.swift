...
@Suite("Work view")
struct WorkViewTests {
    @Test
    func `every view asks SF Symbols for the shape the design drew`() {
        #expect(WorkView.allOpen.symbol == "circle.fill")
        #expect(WorkView.unblocked.symbol == "circle")
        #expect(WorkView.inProgress.symbol == "diamond.fill")
        #expect(WorkView.blocked.symbol == "triangle")
    }
    ...
    func `no two views draw the same mark`() {
        #expect(Set(WorkView.allCases.map(\.symbol)).count == WorkView.allCases.count)
    }
