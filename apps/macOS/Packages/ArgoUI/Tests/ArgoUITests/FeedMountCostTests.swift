...
struct FeedMountCostTests {
    ...
    /// the mount lays out.
    private static func remounted(over geometry: FeedGeometry) -> FeedTableCoordinator {
    ...
    @Test
    func `a reading that opens at its end is laid out once`() throws {
        ...
        _ = Self.remounted(over: geometry)
        let coordinator = Self.remounted(over: geometry)
