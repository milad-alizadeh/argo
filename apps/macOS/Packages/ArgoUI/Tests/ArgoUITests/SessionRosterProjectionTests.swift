...
struct SessionRosterProjectionTests {
    ...
    func `an archived Session is absent from the roster`() {
        let sessions = [
            session(id: "kept"),
            session(id: "cleared", isArchived: true),
        ]
    ...
    func `new activity in an archived Session does not put it back on the roster`() {
        ...
        let sessions = [
            session(id: "kept", lastSeenAtMs: msAgo(3600)),
            session(id: "cleared", status: .running, lastSeenAtMs: msAgo(0), isArchived: true),
        ]
    ...
    func `an archived Session is described exactly as a kept one is`() throws {
        ...
        let cleared = session(id: "cleared", branch: branch, lastSeenAtMs: msAgo(120))
    ...
    func `the foot names how many Sessions are behind it, and is absent when none are`() {
        let sessions = [session(id: "a", isArchived: true), session(id: "b", isArchived: true)]
