...
struct MinimapTableTests {
    ...
    private static func rects(_ header: [String], _ rows: [[String]]) -> [MinimapRowRect] {
        table(header, rows).laid(across: measure).rects
    }

    @Test
    func `every cell is drawn, in the table's own ink`() {
        let rects = Self.rects(["a", "b", "c"], [["1", "2", "3"]])
        #expect(rects.count == 6)
        #expect(rects.allSatisfy { $0.ink == .table })
        #expect(rects.allSatisfy { $0.drawn == .frame })
    }
    ...
    func `a column of long cells is drawn wider than a column of short ones`() {
        let rects = Self.rects(["a", "b"], [["1", "a much longer cell than its neighbour"]])
        #expect(Self.width(rects[1]) > Self.width(rects[0]) * 3)
    }
    ...
    func `the cells of one row meet edge to edge across the whole measure`() {
        let rects = Self.rects(["one", "two", "three"], [])
        #expect(rects[0].from == 0)
        #expect(abs(rects[2].to - Self.measure) < 0.0001)
        #expect(abs(rects[0].to - rects[1].from) < 0.0001)
        #expect(abs(rects[1].to - rects[2].from) < 0.0001)
    }
    ...
    func `a row holding a cell that wraps stands taller than one that does not`() {
        ...
        let rects = Self.rects(["a", "b"], [["1", long], ["2", "3"]])
        let rows = Dictionary(grouping: rects, by: \.y).sorted { $0.key < $1.key }
        #expect(rows.count == 3)
    ...
    func `every cell of a row stands at the row's own height`() {
        ...
        let rects = Self.rects(["a", "b"], [["1", long]])
        let body = rects.filter { $0.y > 0 }
        #expect(body.count == 2)
...
private extension MinimapTableTests {
    static func width(_ rect: MinimapRowRect) -> CGFloat {
        rect.to - rect.from
    }
}
