...
/// A single-line row reported as the pieces it is drawn in — a call's mark, its verb, what it
...
struct MinimapLineMarkTests {
    private static let measure: CGFloat = 720 - ArgoFeedRow.inset * 2

    private static func marks(_ parts: [MinimapLinePart], across measure: CGFloat = measure)
        -> [MinimapRowMark] {
    ...
    func `the pieces are laid end to end with the row's own gap between them`() {
        let marks = Self.marks(Self.call(added: 30, removed: 10))
        #expect(marks.count == 5)
        #expect(marks[0].from == 0)
        #expect(marks[0].to == ArgoFeedRow.callSymbolWidth)
    ...
    func `each piece is drawn in its own ink and stands one line tall`() {
        let marks = Self.marks(Self.call(added: 30, removed: 10))
        #expect(marks.map(\.ink) == [.command, .command, .command, .added, .removed])
        #expect(marks.allSatisfy { $0.y == 0 })
        #expect(marks.allSatisfy { $0.height == ProseFace.body.lineBox })
    }

    /// The counts are as wide as their digits, so a bigger patch is a wider mark. The fixed slab
    /// reported the same width for `+1` and `+2000`.
    @Test
    func `a bigger count is drawn wider than a smaller one`() {
        let small = Self.marks(Self.call(added: 1, removed: 1))
        let large = Self.marks(Self.call(added: 2000, removed: 1))
    ...
    func `nothing is drawn past the measure`() {
        ...
        #expect(marks.allSatisfy { $0.to <= Self.measure })
        #expect(marks.count == 1)
    }

    @Test
    func `a column not yet laid out still reports an ordered mark`() {
        let marks = Self.marks(Self.call(added: 1, removed: 1), across: 0)
        #expect(marks.allSatisfy { $0.from <= $0.to })
    }

    private static func width(_ mark: MinimapRowMark) -> CGFloat {
        mark.to - mark.from
    }
}
