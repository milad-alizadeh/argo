...
struct WorkBacklogBandsTests {
    ...
    func `the unread band comes last`() {
        let reading = WorkFixture.reading { priorities in priorities = [607: "high"] }
    ...
    func `a word Argo has no band for keeps its own header`() {
        let reading = WorkFixture.reading { priorities in priorities[275] = "critical" }
        let room = WorkRoomProjection.room(from: reading)
    ...
    func `two spellings of one word are one band`() {
        let reading = WorkFixture.reading { priorities in priorities[275] = "Low" }
        let room = WorkRoomProjection.room(from: reading)
    ...
    func `the unread band and an empty word do not share a key`() {
        let reading = WorkFixture.reading { priorities in priorities = [275: ""] }
        let room = WorkRoomProjection.room(from: reading)
