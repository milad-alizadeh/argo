...
struct PriorityHeader: View {
    ...
    let band: WorkRoomProjection.Band
...
#Preview("Priority headers — the three bands, and the one nobody read") {
    let roots = WorkFixture.room.backlog

    VStack(alignment: .leading, spacing: ArgoSpacing.flush) {
        ForEach(WorkRoomProjection.bands(of: roots)) { band in
            PriorityHeader(band: band, count: WorkRoomProjection.drawn(band, shut: []).count)
        }
        PriorityHeader(band: WorkRoomProjection.Band(priority: nil, roots: []), count: 0)
    }
