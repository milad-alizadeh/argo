...
/// EVERY row of the document rather than for the ones on screen — so a 4 000-row reading paid 4 000
...
/// both held against what SwiftUI draws by `MinimapBlockHeightTests`. What is added here is the two
/// things a ROW has that a block does not: the whole point each block's baselines land on, and the
/// Turn's copy chip. `FeedTypesetHeightTests` holds the total against the ruler at zero tolerance.
///
/// PROSE only, and everything else stays on the ruler: a bubble's chrome, a call's three columns, a
/// gallery's plates, and the two markdown blocks that size THEMSELVES (a table, a diagram) are not
/// glyphs, so Core Text has no answer for them and a guess overlaps rows.
@MainActor
enum FeedRowMeasure {
