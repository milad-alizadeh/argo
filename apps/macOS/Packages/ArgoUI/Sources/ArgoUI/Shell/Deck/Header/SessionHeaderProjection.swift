...
package enum SessionHeaderProjection {
    package struct Header: Equatable, Sendable {
        ...
        /// to say (#894).
        let issue: IssueRow?
        /// The same branch's pull request, drawn on the tab line's leading edge beside the Ticket
        /// link (#1592). `nil` for a branch with none open.
        ...
        fileprivate init(
            ...
            self.pullRequest = identity.pullRequest
        ...
        var announcement: String {
            ([title, agent, issue?.label, pullRequestWord, checkout?.detail]
                + marks.map(\.detail)
                + [access?.word])
                .compactMap(\.self)
                .joined(separator: ", ")
        ...
        /// `PR #1589`, the same spelling the tab line draws — a screen reader that never visited
        /// the tab line's own link must still hear it (#1592).
        private var pullRequestWord: String? {
            pullRequest.map { "PR #\($0.number)" }
        }
