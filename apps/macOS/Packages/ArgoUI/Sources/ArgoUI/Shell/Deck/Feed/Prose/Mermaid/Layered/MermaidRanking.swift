...
///
/// A layered diagram is not always acyclic — `Read --> Write --> Read` is a source people really
/// write — and a ranking pass that meets a cycle either hangs or ranks for ever. So it is BROKEN
...
struct MermaidRanking: Equatable, Sendable {
    ...
    /// The edges the walk had to turn around, by their index in the graph's own list.
...
extension MermaidGraph {
    /// The graph ranked by longest path over its acyclic part.
    ///
    ...
    func ranking() -> MermaidRanking {
        ...
        // graph this pass has misjudged from spinning rather than merely drawing oddly.
    ...
    /// its own stack. Walked in the source's order from every node in turn, so the same graph
    /// breaks at the same edges every run.
    private func backEdges() -> Set<Int> {
        var walk = MermaidCycleWalk(graph: self)
        for name in names {
            walk.visit(name)
        }
