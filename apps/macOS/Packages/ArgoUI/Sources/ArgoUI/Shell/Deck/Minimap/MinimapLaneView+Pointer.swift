...
package extension MinimapLaneView {
    ...
    internal func attach(to feed: FeedTableHandle, within turns: Int = 3) {
        self.feed = feed
        // The one change no notification of the feed's carries — see `FeedTableHandle.coordinator`.
        // Both captures are weak: the handle holds this closure, and it belongs to the deck above
        // both of these views.
        ...
        // Another DECK under this lane — the reader switched Session, or scoped the rail onto a
        // Subagent (`KeptDecks`). A height this lane has already answered says nothing about the
        // document it is now beside, so it is forgotten here rather than left to swallow the fresh
    ...
    internal func stopWatchingModifiers() {
    ...
    internal func readModifiers(_ flags: NSEvent.ModifierFlags) {
