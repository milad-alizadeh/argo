...
extension FeedTableCoordinator {
    ...
    ///
    /// What used to hang here was the pair: the whole cache emptied, and then EVERY row noted in
    /// one block. `noteHeightOfRows` makes AppKit ask the delegate for each noted height
    /// synchronously, every one was now a miss, and every miss is a full SwiftUI layout against the
    /// ruler — O(N) of them inside a single frame, up to 1.4 s of frozen main thread (#856).
    ///
    /// So the drop moves with the noting. The rows on screen are surrendered and noted at once, so
    /// the viewport is correct in the very next frame; every other row keeps the stale height it
    /// rode the drag on until the batch that squares it up. A stale height is what the reader
    /// already sees, and it costs nothing to ask for.
    ///
    ...
    private func remeasureEverything(on table: NSTableView) {
        ...
        tailing = Task { [weak self] in await self?.measure(tail) }
    }

    /// One batch a run-loop turn until the tail is measured, or a fresher re-measure cancels it.
    ///
    /// The reading is read back and put where it was around every batch: the rows ABOVE the reader
    ...
    private func measure(_ tail: IndexSet) async {
        var pending = tail
        while !pending.isEmpty {
            guard !Task.isCancelled, let table else { return }
            let taken = IndexSet(pending.prefix(Self.remeasureBatch))
            pending.subtract(taken)
            let batch = taken.filteredIndexSet { shown.indices.contains($0) }
            ...
            // are asked for at the next layout, and leaving that to whenever one happens would
            // pile every batch's askings back into one block.
            scroller?.layoutSubtreeIfNeeded()
