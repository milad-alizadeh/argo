...
extension FeedTableCoordinator {
    ...
    /// ends.
    ///
    /// A press is not a burst and it is not a document: a fold is a handful of rows and every one
    /// of
    /// them is arithmetic (`FeedShapeHeight.folded`), so there is nothing here worth a hop off the
    /// main actor — and the hop is what the reader sees. `touchUp` has already redrawn the cell in
    /// its open shape, so a height landing a hop later leaves the rows below it at the closed
    /// height
    /// for that hop and then steps them down to the open one. Measured with the fix reverted, the
