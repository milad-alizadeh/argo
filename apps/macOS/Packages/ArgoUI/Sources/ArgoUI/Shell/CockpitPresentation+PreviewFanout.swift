...
extension CockpitPresentation.Session {
    ...
    static let fannedOut: [TranscriptEvent] = [
        .toolCall(ToolCall(
            ...
            target: "research: how the study inks an attention row", atMs: nil,
        )),
        .toolCall(ToolCall(
            id: "fan-verify", name: "Task", kind: .delegate,
            target: "verify: the fold breaks at every mark", atMs: nil,
        )),
        ...
        .toolCall(ToolCall(
            ...
            target: "sweep: every surface that reads a stop reason", atMs: nil,
        )),
