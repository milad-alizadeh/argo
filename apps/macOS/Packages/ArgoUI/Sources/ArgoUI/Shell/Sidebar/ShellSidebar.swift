...
struct ShellSidebar: View {
    ...
    var body: some View {
        ...
        // The order is taken at the moment the window comes up and released only when it goes
        // away, so what re-settles is a roster nobody is in front of. Reading the rows already
        // published makes the freeze a fixed point rather than a fresh sort of its own.
