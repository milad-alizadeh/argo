...
/// states are shot from `deliveryDots` and from the `TicketsPanesSpecimen` renders, which take a
/// reading directly.
struct TicketsRoomSpecimen: View {
    @State private var navigation = CockpitNavigationModel.pointedAtTickets
...
/// shell cannot be driven into, since its Tickets room is fed from one fixture.
///
/// The titlebar is what it gives up, which is chrome the room does not decide.
struct TicketsPanesSpecimen: View {
    private let reading: TicketsReading

    @State private var cockpitRoom = CockpitRoom.tickets
    ...
    @State private var view: TicketsView
    ...
    var body: some View {
        ...
        HStack(spacing: ArgoSpacing.flush) {
            ...
            if tickets.room.vacancy != .unbound {
                tickets.sidebar
...
struct DeliveryChipsSpecimen: View {
    var body: some View {
        VStack(alignment: .leading, spacing: ArgoTicketDetail.chipGap) {
            ForEach(TicketsFixture.everyChip) { DeliveryChip(delivery: $0) }
        }
