...
/// wherever it appears. What keeps one from standing proud of its line is the ceiling the contract
/// asserts: no rung outgrows the densest text role it may sit next to.
public enum ArgoIconSize: CGFloat, Sendable, CaseIterable {
    ...
    /// a branch beside its name. The workhorse, and the right answer unless the mark IS the
    /// control.
    ///
    /// A chevron sits here too, on the same rung as the marks it shares a line with. It had a
    /// smaller rung of its own for a while, on the theory that a pointer is punctuation rather than
    /// a word — at 6pt it was a control nobody could see they were allowed to click.
    case inline = 10
