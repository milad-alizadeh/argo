...
#Preview("Backlog list — everything open") {
    @Previewable @State var selection = RowSelection(one: 272)
    @Previewable @State var shut: Set<Int> = []

    BacklogList(
        rows: TicketsFixture.room.backlog,
        held: .init(picking: .init(selection: $selection), shut: $shut),
    )
...
#Preview("Backlog list — a parent folded, and its header's count with it") {
    @Previewable @State var selection = RowSelection(one: 607)
...
#Preview("Backlog list — nobody read a priority") {
    ...
    BacklogList(
        rows: unread,
        held: .init(picking: .init(selection: .constant(RowSelection())), shut: $shut),
    )
...
#Preview("Backlog list — closed, flat, with another page behind it") {
    @Previewable @State var selection = RowSelection(one: 264)
    @Previewable @State var shut: Set<Int> = []
    ...
    BacklogList(
        rows: room.backlog,
        held: .init(picking: .init(selection: $selection), shut: $shut),
        header: TicketsChromeProjection.reading(of: room, in: .closed, showing: selection.last),
        more: {},
    )
    .frame(width: ArgoBacklogList.width, height: 420)
    .argoDeckSurface()
