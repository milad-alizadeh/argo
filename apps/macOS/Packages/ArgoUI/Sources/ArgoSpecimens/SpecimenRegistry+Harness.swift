...
extension SpecimenRegistry {
    ...
    private static func harness(_ harness: AgentCLI, editable: Bool) -> ComposerSpecimen {
        ...
        let composer = SessionComposerProjection.Composer(
            ...
            facts: facts, standingAllows: [], isRunning: !editable,
            mode: .exactly(.code, cli: "acceptEdits"), modeDidNotTake: nil, lostTurn: nil,
            canAttach: true, canRunCommands: harness == .claude,
        )
        var specimen = ComposerSpecimen(composer: composer, opening: .runSettings)
