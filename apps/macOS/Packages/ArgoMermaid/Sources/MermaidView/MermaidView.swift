...
public struct MermaidView: View {
    ...
    public var body: some View {
        ...
        // `MermaidCaptionLayout` places one subview per caption and the suites assert the
        // pairing — but one of them is memoised on the source and the other is recomputed on
        // every body. A Gantt
        ...
        .frame(height: MermaidPlans.of(diagram).size.height)
    }
    ...
    private func figures(_ ink: MermaidInk) -> some View {
        Canvas { context, _ in
            MainActor.assumeIsolated {
                MermaidDrawing(plan: MermaidPlans.of(diagram), ink: ink).draw(in: &context)
            }
        }
    }
}
