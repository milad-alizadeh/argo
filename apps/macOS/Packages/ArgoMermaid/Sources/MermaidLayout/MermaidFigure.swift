...
public struct MermaidFigure: Equatable, Sendable {
    ...
    package var weight: Weight = .full

    /// The box this figure occupies, in the plan's own coordinates. The whole of what the overview
    /// lane reads off a figure, and public for that reason: the lane maps a silhouette without
    /// learning what a form or a role is.
    public var bounds: CGRect {
        form.bounds
    }

    /// Whether this mark joins two things rather than being one. The lane draws a connector as a
    /// bar and everything else as a frame, because a stroked hairline at its scale is nothing at
    /// all — and that is the only distinction it needs.
    ...
    package enum Form: Equatable, Sendable {
    ...
    package enum Weight: Equatable, Sendable {
        case spent, ordinary, full
    }
}

package extension MermaidFigure.Form {
