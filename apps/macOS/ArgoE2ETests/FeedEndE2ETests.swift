...
/// middle of it — so this is the state the anchor `FeedMotionE2ETests` covers does not describe.
///
...
final class FeedEndE2ETests: FeedE2ECase {
    ...
    /// while the reading has stopped following, so a control that comes back after a drag nobody
    /// scrolled during is the feed reporting that it lost the end.
    func testTheEndOfTheReadingSurvivesASeamDrag() {
        ...
        // an animated transition, so a feed that dropped the end and offered the way back a frame
        // or two later would pass a bare `exists` taken the instant the drag ended.
        XCTAssertFalse(
