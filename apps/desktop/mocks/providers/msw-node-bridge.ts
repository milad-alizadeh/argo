// The one Mock Service Worker server this process runs, and the plumbing that lets a mock
...
// interception of `fetch`, so every mock provider layers its routes onto this one instance rather
// than owning a server of its own; a test that runs two mocks at once (the Account harness) needs
// both alive together.
...
function theServer() {
  ...
  server.listen({
    onUnhandledRequest(request, print) {
      // A request to a mock provider's own origin that matched no route is the silent-404 bug this
...
export type MockMount = {
  origin: string
  requests: string[]
  routes: Record<string, NodeRoute>
}

// Layers one mock provider's routes onto the shared server, and returns the way it retires them:
// calling it back flips a private flag so a later request to a closed mock falls through as
// unhandled rather than answer state a fresh mock, started under the same fixed origin, replaced.
export function mountMockProvider(mount: MockMount): () => void {
  ...
  const handlers: HttpHandler[] = Object.entries(routes).map(([key, run]) => {
    ...
    return METHODS[method](`${origin}${path}`, async ({ request }) => {
      ...
      try {
        ...
        return Response.json({ error: 'mock_failed' }, { status: 500 })
      }
    })
  })
...
// Asserts the AC every mock provider shares: a `GET` this mock never stubbed rejects the caller,
