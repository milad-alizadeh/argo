...
test('names every packaged test command as a test', () => {
  expect(scripts['test:packaged-contents']).toContain('scripts/assert-packaged-pty.mts')
  expect(scripts['test:packaged-manifest']).toBe('bun run desktop:manifest')
  expect(scripts['test:packaged-pty']).toContain('scripts/prove-packaged-pty.mts')
