...
if (!pagePath || !outputDirectory || states.length === 0) {
  process.stderr.write('usage: render-design-page.mts <page.html> <output-dir> <state>...\n')
  process.exit(2)
}
...
// Where a capture is written and what it loads: fixed for the whole render, unlike `state`.
type CaptureTarget = { outputDirectory: string; pageURL: string }
...
async function capture(
  ...
  const { outputDirectory, pageURL } = target
  const [urlTarget = '', appearance = 'dark'] = state.split(':')
  const [fragment, width = String(VIEWPORT.width)] = urlTarget.split('@')
  if (!/^\d+$/.test(width))
...
async function renderAll(page: string, outputDirectory: string, states: string[]): Promise<void> {
  const pageURL = pathToFileURL(path.resolve(page)).href
  await mkdir(outputDirectory, { recursive: true })
  const window = openWindow()
  for (const state of states)
    process.stdout.write(`${await capture(window, { outputDirectory, pageURL }, state)}\n`)
}

app.whenReady().then(() =>
  renderAll(pagePath, outputDirectory, states).then(
    () => app.exit(0),
    (error) => {
