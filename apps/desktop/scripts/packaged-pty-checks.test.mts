...
// trees next door, in `packaged-pty-fixtures.test.mts`.
import { describe, expect, test } from 'bun:test'
import { existsSync, readFileSync } from 'node:fs'
import path from 'node:path'
import { FuseV1Options } from '@electron/fuses'
import { CYCLES, RESULT_PREFIX, SKIP_ENDURANCE_ENV } from './acceptance-protocol.mts'
import { PRODUCTION_FUSE_PROFILE } from './fuse-profile.mts'
...
const PROTOCOL_USERS = [
  'src/main.ts',
  'src/platform/main/pty-acceptance/pty-acceptance.ts',
  'scripts/prove-packaged-pty.mts',
  'scripts/packaged-app.mts',
]
...
function atLeastVersion(version: string, floor: number[]): boolean {
...
describe('the root command surface', () => {
  ...
  const developmentLauncher = () => read(path.join(desktopRoot, 'scripts', 'dev-instance.mts'))
  ...
  test('runs the app and Storybook as persistent tasks', () => {
    ...
    expect(desktop().scripts.dev).toBe('node scripts/dev-instance.mts')
  ...
  test('links the hoisted Electron before Forge starts', () => {
    ...
    const linked = launcher.indexOf('scripts/link-hoisted-electron.mts')
    expect(linked).toBeGreaterThan(-1)
    expect(linked).toBeLessThan(launcher.indexOf("spawn('electron-forge'"))
    expect(existsSync(path.join(desktopRoot, 'scripts', 'link-hoisted-electron.mts'))).toBe(true)
  })
...
describe('forge.config.ts', () => {
  ...
  // The fuse values moved out to `fuse-profile.mts` under #1807, so that the config and the
