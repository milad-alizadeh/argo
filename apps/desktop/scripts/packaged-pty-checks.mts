...
// actually be chosen.
export function nativeDirs(arch: string): string[] {
...
export function archOf(outputPath: string): string | null {
...
export function archOfBinary(appPath: string): string | null {
...
function statOrNull(target: string): import('node:fs').Stats | null {
  try {
...
export function asarEntryList(asarPath: string): string[] {
  // `isPack` asks for the pack-file listing; this reads the archive's own entries.
  return asar.listPackage(asarPath, { isPack: false }).map((entry) => entry.replace(/^[\\/]/, ''))
}

function asarEntries(asarPath: string): Set<string> {
  return new Set(asarEntryList(asarPath))
}

function presenceFailures(
  asarPath: string,
  arch: string,
): { failures: string[]; chosen: string | null } {
...
export function unpackedFailures(appPath: string, chosen: string): string[] {
...
async function fuseFailures(appPath: string): Promise<string[]> {
  const binary = executableIn(appPath)
  ...
  const read = actual === undefined ? 'nothing' : (FuseState[actual] ?? actual)
  return [
    `fuse OnlyLoadAppFromAsar reads ${read}, expected ENABLE. Unpacking a ` +
...
export async function packagedPtyFailures(appPath: string, arch: string | null): Promise<string[]> {
