...
/** One packaged file, named as the failure would name it, with its bytes. */
type BundledFile = { name: string; contents: Buffer }

// Every file in the archive and beside it, as bytes: a bundler keeps a string literal whatever it renames.
function* bundledFiles(appPath: string): Generator<BundledFile> {
...
export function mockCliFailures(appPath: string): string[] {
