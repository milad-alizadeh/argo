#!/usr/bin/env node
// `node scripts/audit-release.mts <directory of downloaded release assets>` — the backstop's
...
import { releaseAudit } from './release-audit.mts'
import { artifactDigests, VERDICT_FILENAME } from './release-verdict.mts'

const directory = process.argv[2]
if (!directory) {
  process.stderr.write('usage: audit-release.mts <directory of downloaded release assets>\n')
  process.exit(2)
}
...
if (verdictPath) {
  try {
    ...
    process.stdout.write(
      `${JSON.stringify({ case: 'verdict missing', reasons: [`release-verdict.json could not be read: ${(error as Error).message}`], digests: [] })}\n`,
    )
    process.exit(0)
  }
}
