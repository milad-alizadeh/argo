...
import { packageManifestAssertion } from './package-manifest.mts'
import type { Assertion } from './release-assertions.mts'
import {
...
} from './release-assertions.mts'
import type { SigningStatus } from './signing-readback.mts'

export const VERDICT_FILENAME = 'release-verdict.json'

/** One artifact the verdict covers, keyed by the bytes it judged. */
export type ArtifactDigest = { name: string; bytes: number; sha256: string }

/** The whole document, as ADR-0036 fixes it. */
export type Verdict = {
  version: string
  tag: string
  app: string
...
export function artifactDigests(filePaths: string[]): ArtifactDigest[] {
  return filePaths
    .map((filePath) => ({
...
export function verdictFailures(verdict: Pick<Verdict, 'assertions'>): string[] {
