#!/usr/bin/env node
// `node scripts/changelog-entry.mts <version> [--changelog <file>]` — prints the release notes for
...
if (import.meta.main) {
  ...
  const changelogPath = args.includes('--changelog')
    ? (args[args.indexOf('--changelog') + 1] ?? CHANGELOG_PATH)
    : CHANGELOG_PATH
  if (!version) {
    process.stderr.write('usage: changelog-entry.mts <version> [--changelog <file>]\n')
    process.exit(2)
  }
