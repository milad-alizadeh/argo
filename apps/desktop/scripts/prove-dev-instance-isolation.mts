...
import { type DevelopmentInstance, developmentInstance } from './dev-instance.mts'
...
async function waitFor<T>(check: () => Promise<T>, description: string): Promise<T> {
...
function run(worktree: string, args: string[], stdio: StdioOptions = 'pipe'): ChildProcess {
  return spawn('bun', args, { cwd: worktree, stdio })
}

async function status(worktree: string): Promise<ReadyRecord> {
...
async function readyStatus(worktree: string): Promise<ReadyRecord> {
...
async function stop(worktree: string): Promise<void> {
  const child = run(worktree, ['run', '--cwd', 'apps/desktop', 'dev:stop'])
  const code = await new Promise((resolve) => child.once('exit', resolve))
  if (code !== 0) throw new Error(`desktop:stop failed for ${worktree}`)
}

async function absent(instance: DevelopmentInstance, processId: number): Promise<void> {
  await waitFor(async () => {
...
async function main() {
  ...
  try {
    ...
    for (const field of ['port', 'title', 'userData', 'worktree'] as const) {
