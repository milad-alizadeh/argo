...
import { entitlementKeysFor } from './entitlements.mts'

/** What the shipped signature says about the app as a whole. */
...
/** One signed path: the keys it carries beside the keys #1771 allows it. */
...
function readVerbose(command: string, args: string[]): { ok: boolean; output: string } {
...
export function readSigningStatus(appPath: string): SigningStatus {
...
export function signablePaths(appPath: string): string[] {
...
function entitlementsXml(filePath: string): string | null {
  try {
    ...
    const failed = error as { stderr?: unknown; message?: string }
    const stderr = String(failed.stderr ?? '')
    if (stderr.includes(UNSIGNED)) return null
    throw new Error(`codesign could not read ${filePath}: ${stderr.trim() || failed.message}`)
  }
}
...
export function grantedKeys(xml: string): string[] {
  if (!xml.trim()) return []
  const json = execFileSync('plutil', ['-convert', 'json', '-o', '-', '-'], {
    encoding: 'utf8',
    input: xml,
  })
  const entitlements = JSON.parse(json) as Record<string, unknown>
...
export function readEntitlements(appPath: string, appName: string): EntitlementReading[] {
  const readings: EntitlementReading[] = []
...
export function entitlementFailures(readings: EntitlementReading[]): string[] {
  return readings
