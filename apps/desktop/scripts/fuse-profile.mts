...
/** One fuse, as the shipped binary reads it beside what the profile requires. */
export type FuseReading = {
...
export const PRODUCTION_FUSE_PROFILE: Record<FuseV1Options, boolean> = {
...
const FUSE_NAMES: Record<string, string | undefined> = Object.fromEntries(
...
function isEnabled(state: FuseState | undefined): boolean {
...
export async function readFuseWire(binaryPath: string): Promise<FuseReading[]> {
  const wire = await getCurrentFuseWire(binaryPath)
  return Object.entries(PRODUCTION_FUSE_PROFILE).map(([index, expected]) => {
    const state = wire[Number(index) as FuseV1Options]
    ...
    return {
      ...
      state: (state !== undefined && FuseState[state]) || String(state),
      matches: actual === expected,
    }
  })
}

export function fuseWireFailures(readings: FuseReading[]): string[] {
  return readings
