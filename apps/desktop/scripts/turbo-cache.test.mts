...
/** As much of one `turbo.json` task as these assertions read. */
type TurboTask = {
  cache?: boolean
  inputs?: string[]
  outputs?: string[]
...
/** The whole file, in the two keys this suite reads. */
type TurboConfig = { globalDependencies: string[]; tasks: Record<string, TurboTask> }
...
describe('what turbo caches', () => {
  ...
  test('hashes the root files the desktop tasks read', () => {
    expect(task('test').inputs).toContain('$TURBO_ROOT$/docs/design/tokens.css')
    expect(task('test').inputs).toContain('$TURBO_ROOT$/scripts/design-token-*.mts')
    expect(task('typecheck').inputs).toContain('$TURBO_ROOT$/scripts/design-token-*.mts')
  })
  ...
  test('keeps the packaged PTY test out of every other task graph', () => {
    for (const [name, entry] of Object.entries(turbo().tasks))
...
/** One task of turbo's `--dry=json`, in the two keys these assertions read. */
type DryRunTask = { task: string; inputs: Record<string, string>; dependencies: string[] }

let dryRun: DryRunTask[] | undefined
function resolved(task: string): { files: string[]; dependencies: string[] } {
  if (!dryRun) {
    const run = spawnSync(
      'bun',
  ...
  if (!found) throw new Error(`turbo's dry run resolved no ${task} task`)
...
describe('what the packaged tasks hash', () => {
  ...
  test.each([
    ['test:packaged-contents', 'scripts/packaged-mock-cli-checks.mts'],
