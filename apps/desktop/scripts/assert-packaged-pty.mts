...
// `packaged-pty-checks.mts`; this is the two callers of them.
import path from 'node:path'
import process from 'node:process'
import { archOf, archOfBinary, packagedPtyFailures } from './packaged-pty-checks.mts'

function refusal(appPath: string, failures: string[]): string {
  return [
...
export async function assertPackagedPty(outputPaths: string[], appName: string): Promise<void> {
...
if (process.argv[1] === import.meta.filename) {
  ...
  const { mockCliFailures } = await import('./packaged-mock-cli-checks.mts')
  const { packagedApp, SHIPPED_ARCHES } = await import('./packaged-app.mts')
