...
export function containerReading(container: HTMLElement) {
  ...
  return {
    ...
    // it. A row that sets its own face — the code row, off `--font-mono` — is not in this string,
    // so a face changed for code alone would not invalidate the reading. Both faces move together
    // today, from `tokens.css`; the day one moves alone, it belongs in the key.
    font: style.font,
