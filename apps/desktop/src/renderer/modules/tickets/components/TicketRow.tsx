...
import { StatusMark } from './TicketStatus'
...
type TicketRowProps = BacklogRow & {
  // The key column's width class, one per provider (`lib/sources.ts`).
  keyColumn: string
  selected: boolean
  now: number
  onSelect: () => void
...
// The key column is one width, so every title starts on one line and a child indents from it.
export function TicketRow(props: TicketRowProps) {
  const { ticket, depth, parent, keyColumn, selected, now, onSelect } = props
  ...
  return (
    <Item
      ...
      size="xs"
    >
      <span className={`${keyColumn} shrink-0 font-mono type-meta text-faint`}>{ticket.key}</span>
      {ticket.status ? <StatusMark named={false} status={ticket.status} /> : null}
      <span
