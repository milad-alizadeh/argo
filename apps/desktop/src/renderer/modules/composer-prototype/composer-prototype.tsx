...
import { FeedPermission } from './feed/feed-attention'
import {
  FeedEvidencePrototype,
  type FeedPrototypeEvidence,
  SessionFeedPrototype,
} from './session-feed-prototype'
import {
...
const MENTION_SUGGESTIONS: ComposerSuggestion[] = [
  {
    label: 'composer-prototype.tsx',
    value: '@composer-prototype.tsx',
...
function PrototypeSessionRoster({
  ...
  return (
    <aside className="flex h-full min-h-0 w-full min-w-84 flex-col bg-sidebar">
      ...
      />
      <div className="-mt-px flex min-h-0 flex-1 flex-col border-t border-border/60 bg-sidebar">
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" side="top" className="w-60">
        <DropdownMenuGroup>
          ...
          <DropdownMenuItem onClick={() => add('composer-prototype.tsx')}>
            <File />
            File reference
          </DropdownMenuItem>
...
function initialAttachments() {
  if (new URLSearchParams(window.location.search).get('test') !== 'attachments-10') {
    return ['workspace.jpg', 'composer-prototype.tsx']
  }
  return [
    'workspace.jpg',
    'composer-prototype.tsx',
    'queue.ts',
    'context.tsx',
    'permissions.ts',
    'models.json',
...
function ContextSurface({
  ...
  if (layout === 'footer') {
    return (
      <div className="flex items-center gap-3 border-t bg-sidebar px-3 py-2.5">
  ...
  return (
    <div className="mb-2 rounded-xl border bg-card px-4 py-3 shadow-sm">
