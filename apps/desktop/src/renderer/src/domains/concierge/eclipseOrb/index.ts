...
export type { OrbHandle, OrbOptions, OrbState } from './types'
...
export function createEclipseOrb(canvas: HTMLCanvasElement, options: OrbOptions = {}): OrbHandle {
  ...
  // ── Scene palette (fixed) ───────────────────────────────────────────────────
  // The v1 conversation-mode warm swap is dropped; the base colour is constant.
  const base: [number, number, number] = [...P.base]
