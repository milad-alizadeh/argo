import { CheckIcon, CircleIcon, CircleNotchIcon, type IconAtom } from '@/shared/components/ui/icons'
...
type PhasePresentation = {
  ...
  ink: string
  /** The phase NAME's ink in the group header. Only a future phase dims its own name; a
   * done or worked phase keeps the muted role and lets the rail carry the state. */
  nameInk: string
  /** The fragment's ink in a collapsed Run's phase walk. Only the phase being worked is
...
// One hue per state, decided here and nowhere else — the rail, the status word, the phase
// name and the walk fragment all read their class out of this row. `wait` deliberately
// carries no hue at all: faint IS its value. Classes are written out per state rather than
// interpolated so Tailwind's scanner still sees each one literally.
export const PHASE_PRESENTATION: Record<PhaseState, PhasePresentation> = {
  done: {
    word: 'done',
    rail: 'border-tone-done/55',
    ink: 'text-tone-done',
    nameInk: 'text-muted-foreground',
    walkInk: 'text-foreground-faint',
    glyph: CheckIcon,
  },
  run: {
    word: 'running',
    rail: 'border-tone-run',
    ink: 'text-tone-run',
    nameInk: 'text-muted-foreground',
    walkInk: 'text-tone-run',
