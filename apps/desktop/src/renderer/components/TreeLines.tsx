...
const anchor = { top: 'var(--ticket-tree-anchor)' } satisfies CSSProperties
const anchorHeight = { height: 'var(--ticket-tree-anchor)' } satisfies CSSProperties
const stemAnchor = {
  top: 'calc(var(--ticket-tree-anchor) + var(--size-icon-meta) / 2)',
  ...
  return (
    <>
      <span aria-hidden="true" className="relative w-(--size-icon-control) shrink-0 self-stretch">
        ...
        {own && !runsOn ? (
          <span className={`${LINE} bottom-auto left-1/2 top-0 w-px`} style={anchorHeight} />
        ) : null}
        {own ? <span className={`${LINE} left-1/2 right-0 h-px`} style={anchor} /> : null}
      </span>
...
export function TreeStem() {
  return (
    <span aria-hidden="true" className={`${LINE} left-1/2 -bottom-px w-px`} style={stemAnchor} />
  )
}

// A leaf's branch runs on through the empty chevron column to its title.
export function TreeTwig() {
  return <span aria-hidden="true" className={`${LINE} left-0 right-0 h-px`} style={anchor} />
}
