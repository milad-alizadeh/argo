...
async function harness(adapter: IndexedAdapter) {
  const root = await mkdtemp(path.join(os.tmpdir(), `argo-backfill-${adapter.cli}-`))
...
describe.each(indexedAdapters)('backfill of $cli history through the Session index', (adapter) => {
  ...
  test('resumes backfill after a restart from the boundary it persisted, without re-parsing', async () => {
    ...
    const resumed = open()
    const resumedProgress = await resumed.index.backfillProgress(adapter.cli)
...
describe.each(indexedAdapters)('reconcile of $cli history through the Session index', (adapter) => {
