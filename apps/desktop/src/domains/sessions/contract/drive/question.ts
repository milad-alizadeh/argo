// The CLI-neutral Question every SessionDriveAdapter answers in (ADR-0024, #1841), mirroring
// permission.ts. A Question states only what shared code and the shared renderer need: the
// prompt, its options, and — when a CLI's own question shape has no honest answer in this type
...
// One question, verbatim off the CLI's own asked question (CONTEXT.md L2 · asking). `options` may
// be empty: a CLI can ask a pure free-text question with no offered choices at all.
...
// CLI's own picker draws, 1-based: the offered options, then one more row for "Type something." —
