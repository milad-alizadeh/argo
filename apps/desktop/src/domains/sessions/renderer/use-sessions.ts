...
function useCodexFailureNotice(options: {
  partialFailures:
    | NonNullable<ReturnType<typeof useRosterQuery>['roster']>['partialFailures']
    | undefined
  add: ReturnType<typeof useToastManager>['add']
  title: string
  description: string
}) {
  const { partialFailures, add, title, description } = options
  useEffect(() => {
    reportCodexFailure(partialFailures ?? [], { title, description }, (notice) =>
      add({ ...notice, type: 'error', priority: 'high' }),
    )
  }, [add, description, partialFailures, title])
}
...
export function useSessions(
  ...
  useCodexFailureNotice({
    partialFailures: rosterData?.partialFailures,
    add,
