...
} from '../hooks/use-session-list-window-store'

type Sessions = SessionsListed['sessions']

// The reader's Session list is ordered by the order each Session was first observed in, not by the order
...
export function rememberedSessionListOrder(sessions: Sessions, remembered: readonly SessionId[]) {
...
// The remembered order is per project scope, so switching project reads that project's Session list in
// the order it returns rather than through the order the previous project left behind. It is held in
// the Session list window store rather than in a ref here, because a ref is forgotten when the sidebar
// unmounts and the list then resorted itself back into read order under the reader.
export function useSessionListOrder(sessionList: SessionRoster | null, scope: string | null) {
  const remembered = useRememberedSessionListOrder(scope)
  const remember = useSessionListWindowStore((state) => state.remember)
  const ordered = useMemo(() => {
    if (sessionList === null) return null
    return {
      ...sessionList,
      sessions: rememberedSessionListOrder(sessionList.sessions, remembered),
    }
  }, [sessionList, remembered])
...
export function useOrderedSessions(projectRoot: string | null) {
  ...
  return {
    ...read,
    sessionList: useSessionListOrder(roster, projectRoot),
    sessionListError: rosterError,
  }
}
