...
// consumer of the sessionList.
//
// Three hooks read the Session list (the sidebar, the Session screen and a Ticket's linked Sessions) and
...
// reader had loaded disappeared. Keeping the cursor here makes the window belong to the sessionList
...
export const useSessionListWindowStore = create<SessionListWindowState>((set) => ({
  cursors: {},
  orders: {},
...
export function useSessionListWindowCursor(projectRoot: string | null): string | null {
  return useSessionListWindowStore((state) => state.cursors[scopeKey(projectRoot)] ?? null)
}

export function useRememberedSessionListOrder(scope: string | null): readonly SessionId[] {
  return useSessionListWindowStore((state) => state.orders[scopeKey(scope)] ?? NO_ORDER)
}
