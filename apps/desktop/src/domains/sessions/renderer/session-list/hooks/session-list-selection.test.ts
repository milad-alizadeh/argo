...
import { clickSelection, EMPTY_SESSION_LIST_SELECTION } from './session-list-selection'

const ORDER = ['a', 'b', 'c', 'd', 'e']

describe('clicking a sessionList row with a modifier key', () => {
  test('a plain click selects one row and replaces any prior selection', () => {
    const first = clickSelection(EMPTY_SESSION_LIST_SELECTION, ORDER, {
      id: 'b',
      modifier: 'plain',
    })
  ...
  test('an additive click toggles one row into, and back out of, the set', () => {
    const selected = clickSelection(EMPTY_SESSION_LIST_SELECTION, ORDER, {
      id: 'b',
      modifier: 'plain',
    })
  ...
  test('a range click selects the inclusive span from the anchor', () => {
    const anchored = clickSelection(EMPTY_SESSION_LIST_SELECTION, ORDER, {
      id: 'b',
      modifier: 'plain',
    })
  ...
  test('a range click with no anchor yet ranges against itself', () => {
    const ranged = clickSelection(EMPTY_SESSION_LIST_SELECTION, ORDER, {
      id: 'c',
      modifier: 'range',
    })
