...
export function SubagentBox({ row, onOpen }: { row: SubagentRow; onOpen?: () => void }) {
  ...
  const facts = [
    row.type ?? null,
    row.model ?? null,
    durationText(row.durationMs ?? null),
    spentTokens(row.tokens ?? null, t),
  ]
  const endState = responded && row.state !== undefined ? t(`workState.${row.state}`) : null
  const summary = joined([endState, ...facts])
  const tone =
    row.state === 'completed' || row.state === undefined ? 'text-muted-foreground' : 'text-danger'
  const body = (
    <div className="flex min-w-0 flex-col gap-hair">
      ...
      {summary === '' ? null : <span className={`type-meta ${tone}`}>{summary}</span>}
