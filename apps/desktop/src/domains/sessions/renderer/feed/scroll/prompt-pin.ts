...
// Pins the prompt at the top and keeps the tail-follow hold in sync with the room that pin needs.
export function useFeedPrompt({
  ...
  sessionId: string
  virtualizer: Virtualizer
  updatePromptHold: (virtualizer: Virtualizer, promptIndex: number | null, gap: number) => void
  paddingStart: number
}) {
