...
export function useRosterPage(projectId: string | null, selectedSessionId: string | null) {
  ...
  const roster = useInfiniteQuery(sessionListOptions(projectId, search, status))
  ...
  const selection = useRosterSelection(selectableIds, selectedSessionId)
  return { search, setSearch, status, setStatus, syncStatus, roster, visible, selection }
}
