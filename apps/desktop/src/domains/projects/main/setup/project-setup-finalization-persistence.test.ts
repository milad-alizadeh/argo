...
test('recovers a completed worktree promotion without promoting it twice after a crash', async (context) => {
  ...
  setup.transition('project-1', { type: 'PLAN_VALIDATED', plan })
  setup.transition('project-1', { type: 'ACCEPT_PLAN', acceptedPlan: acceptedPlanFixture(plan) })
  setup.transition('project-1', { type: 'APPLICATION_COMPLETED', finalDiff: 'diff', progress: [] })
  setup.transition('project-1', { type: 'APPROVE_FINAL_DIFF' })
