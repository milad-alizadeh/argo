...
test('stores empty manual targets for later completion', () => {
  ...
  assert.deepEqual(
    registry.command({
      commandId: 'save',
      event: { type: 'Save manual', source: '{"version":1,"targets":{}}' },
      expectedRevision: 1,
      projectId: 'project-1',
    }),
    {
...
test('refuses an accepted plan that drifted from the reviewed plan', () => {
  ...
  registry.transition('project-1', { type: 'Choose agent', harness: 'claude' })
  registry.transition('project-1', { type: 'Plan validated', plan })
