...
function sourceWrittenDuringEveryRead(root: string, writing: () => boolean) {
  ...
  const transcript = path.join(root, '2026', '09', '13', `${SESSION}.jsonl`)
  return {
    ...source,
    readSessionFiles: async (sessionId: string) => {
      if (writing()) {
        await appendFile(
          transcript,
          `${JSON.stringify({
            timestamp: '2026-09-13T09:00:01.000Z',
            type: 'event_msg',
            payload: { type: 'agent_message', message: 'still going' },
          })}\n`,
        )
      }
...
async function liveReader(context: Parameters<typeof tempRoot>[0], writing: () => boolean) {
  ...
  return createSessionReader([sourceWrittenDuringEveryRead(root, writing)])
}
...
test('reads the Feed of a Session its CLI is still writing rather than spinning on it', async (context) => {
  ...
  const reply = await Promise.race([
    ...
    new Promise((resolve) => setTimeout(() => resolve(gaveUp), READ_BUDGET_MS)),
  ])

  assert.notEqual(reply, gaveUp, 'the read never returned against a transcript still being written')
  assert.equal((reply as { type: string }).type, 'session.feed.read')
})
