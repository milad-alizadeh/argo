...
// A GUI-launched process inherits a bare PATH, not the login shell's — so a CLI installed through
...
// Finds `name` on the login shell's PATH, the way a terminal would, for any CLI. Shared by every
// system wiring that spawns a CLI Argo does not bundle (ADR-0021: this is wiring, not one CLI's).
