...
async function subagentFactsFromFile(filePath: string): Promise<Required<SubagentUsageFacts>> {
  const text = await readFile(filePath, 'utf8').catch(() => null)
  if (text === null) return { tokens: null, model: null }
  const facts: Required<SubagentUsageFacts> = { tokens: null, model: null }
  for (const line of text.split('\n')) {
    const reported = subagentFacts(line)
...
export async function readSubagentTokens(
  ...
  return Promise.all(
    subagentIds.map(async (id) => ({
      id,
      ...(pathsById.has(id)
        ? await subagentFactsFromFile(pathsById.get(id) ?? '')
