...
// in-process handlers in `fake-linear.ts` cannot reach it (#2323). The oauth and GraphQL dispatch
// functions are the one source both transports call; only the socket differs.
import { createServer } from 'node:http'
import type { AddressInfo } from 'node:net'
import type { FakeLinear } from './fake-linear'
...
export async function startFakeLinearLoopback(): Promise<FakeLinear> {
