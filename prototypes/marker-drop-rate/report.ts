...
import { type Cap, capLabel } from './contract'
...
function byToken(trials: Trial[]): string {
  ...
  for (const [token, e] of [...tally].sort(
    (a, b) => b[1].dropped - a[1].dropped || b[1].n - a[1].n,
  )) {
...
function substitutionCandidates(trials: Trial[]): string {
  ...
  if (tally.size === 0)
    return '_No untabled equivalents observed — every drop was a genuine loss, not an unlisted paraphrase._'
  const rows = [
    '| expected | reshaper wrote instead | times | genuine equivalent? |',
    '|---|---|---:|---|',
  ]
...
function retryValue(trials: Trial[]): string {
  const models = [...new Set(trials.map((t) => t.model))]
  const rows = [
    '| model | violations | retry rescued | fell back to extractive | retry cost (median) |',
    '|---|---:|---:|---:|---:|',
  ]
  for (const m of models) {
    // Narrow to the retry rather than filtering on it and asserting later — the retry is
    // absent by design on every non-violating trial, so the type is telling the truth.
    const v = trials.flatMap((t) =>
      t.model === m && t.violation && t.retry ? [{ trial: t, retry: t.retry }] : [],
    )
    if (v.length === 0) {
      rows.push(`| ${m} | 0 | — | — | — |`)
      continue
    }
    const rescued = v.filter(({ retry }) => retry.rescued)
    const times = v.map(({ retry }) => retry.ms).sort((a, b) => a - b)
...
function lengthTable(trials: Trial[]): string {
  ...
  for (const m of models) {
    const cells = caps.map((c) => {
      const ws = trials
        .filter((t) => t.model === m && capKey(t.cap) === c && !t.error)
        .map((t) => t.words)
        .sort((a, b) => a - b)
...
function worstLines(trials: Trial[], n = 12): string {
  const bad = trials
    ...
    .sort(
      (a, b) =>
        b.markers.filter((r) => r.verdict === 'missing').length -
        a.markers.filter((r) => r.verdict === 'missing').length,
    )
    .slice(0, n)
  if (bad.length === 0) return '_No violations to inspect._'
  return bad
    .map((t) => {
      const dropped = t.markers
...
function thinkingLeaks(trials: Trial[]): string {
  ...
  if (leaked.length === 0)
    return '_No thinking output leaked — non-thinking mode held on every trial._'
...
${
  errs.length
    ? `\n## Errors\n\n${errs
        .slice(0, 5)
        .map((e) => `- ${e.model} ${capKey(e.cap)} ${e.chunk}: ${e.error}`)
        .join('\n')}`
    : ''
}`)
