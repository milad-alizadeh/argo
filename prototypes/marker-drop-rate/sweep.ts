...
import { CAPS, capLabel, type Cap } from "./contract";
...
import { MODELS, reshape, availableLocalKeys, type ModelId, type ModelSpec } from "./models";
...
type Cell = { readonly chunk: Chunk; readonly model: ModelSpec; readonly cap: Cap; readonly temp: number };
...
async function main() {
  ...
  for (const id of requested) {
    ...
    if (spec.transport === "lmstudio" && !localKeys.some((k) => k.includes(spec.key!))) {
      console.error(`SKIP ${spec.label} — not loaded in LM Studio (have: ${localKeys.join(", ") || "none"})`);
      continue;
    }
    specs.push(spec);
  }
...
export { runCell, extractiveFallback };
