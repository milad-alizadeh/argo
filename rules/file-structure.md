...
- **Layering is one-directional.** Generic/core layers must not import from specific
  ...
  In this project: `main/` never imports from `renderer/` (and the reverse crosses the
  Electron process boundary — go through preload/IPC, never a direct import), and
  within a layer, feature folders never import each other's leaves.
