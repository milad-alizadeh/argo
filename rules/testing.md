...
For this repo's UI that ladder is Testing Library's, in order: `getByRole` →
`getByLabelText` → `getByText`. `getByTestId` is the last resort, and each use is a small
admission that the element isn't reachable the way a screen reader reaches it.
