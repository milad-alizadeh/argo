...
Runner: `Vitest (`--project=unit`)`. End-to-end: `Vitest browser mode over Storybook stories (Playwright Chromium, `--project=storybook`) — the `story tests` CI check`.
...
- Randomize execution order if `Vitest (`--project=unit`)` supports it, so coupling surfaces
