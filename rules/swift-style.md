...
**`#Preview` is the story.** Every view ships previews covering the states it can be in —
empty, loaded, error, long content — the same coverage rule `ui-components.md` states for
Storybook, and for the same reason: a state with no preview is a state nobody has looked
at. A preview that needs a running engine is a view that violated "views take data".
